...
export function toDate(v, format) {
  return hsz.toDate(v, format)
}
export function toDateText(dateTime) {
  return hsz.toDateText(dateTime)
}
export function toFileSize(size) {
  return hsz.toFileSize(size)
}
...
// 代码生成器数据匹配 根据enCode 匹配
export function dynamicTextByenCode(value, options) {
  if (value === "" || value === null || value === undefined){
    return ''
  }
  if (Array.isArray(value)) {
