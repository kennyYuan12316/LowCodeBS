		...
		<div class="HSZ-common-layout-center">
				...
				<el-form @submit.native.prevent>
					<el-col :span="6">
						<el-form-item label="数据唯一标识">
							<el-input v-model="query.F_RequestId" placeholder="数据唯一标识" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="任务类型">
							<num-range v-model="query.F_TaskType"></num-range>
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="起点库位编码">
							<el-input v-model="query.F_StartAreaCode" placeholder="起点库位编码" clearable />	
						</el-form-item>
					</el-col>
					<template v-if="showAll">
					<el-col :span="6">
						<el-form-item label="起点库区编码">
							<el-input v-model="query.F_StartLocCode" placeholder="起点库区编码" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
					...
					<el-col :span="6">
						<el-form-item label="物料类型编号">
							<el-input v-model="query.F_GoodsCode" placeholder="物料类型编号" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="容器编码">
							<el-input v-model="query.F_ContainerCode" placeholder="容器编码" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="请求发送时间">
							<el-date-picker v-model="query.F_RequestTime" type="datetimerange" value-format="timestamp" format="yyyy-MM-dd HH:mm:ss" start-placeholder="开始日期" end-placeholder="结束日期">
							</el-date-picker>
						</el-form-item>
					</el-col>
					</template>
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list">
					<el-table-column prop="F_RequestId" label="数据唯一标识" align="left" />
					<el-table-column prop="F_ClientCode" label="系统标识" align="left" />
					<el-table-column prop="F_ChannelId" label="服务地址" align="left" />
					<el-table-column prop="F_TaskType" label="任务类型" align="left" />
					<el-table-column prop="F_StartAreaCode" label="起点库位编码" align="left" />
					<el-table-column prop="F_StartLocCode" label="起点库区编码" align="left" />
					<el-table-column prop="F_EndAreaCode" label="终点库区编码" align="left" />
					<el-table-column prop="F_EndLocCode" label="终点库位编码" align="left" />
					<el-table-column prop="F_GoodsCode" label="物料类型编号" align="left" />
					<el-table-column prop="F_ContainerCode" label="容器编码" align="left" />
					<el-table-column prop="F_Quantity" label="数量" align="left" />
					<el-table-column prop="F_RequestTime" label="请求发送时间" align="left">
						<template slot-scope="scope">
							{{scope.row.F_RequestTime|toDate("yyyy-MM-dd hh:mm:ss")}}
						</template>
					</el-table-column>
	...
	export default {
	    ...
	    data() {
	        ...
	        query: {
	                ...
	                F_ContainerCode:undefined,
	                F_RequestTime:undefined,
				},
                list: [],
                listLoading: true,
                multipleSelection: [], 
                ...
                columnList: [
                    { prop: 'F_RequestId', label: '数据唯一标识' },
                    { prop: 'F_ClientCode', label: '系统标识' },
                    { prop: 'F_ChannelId', label: '服务地址' },
                    { prop: 'F_TaskType', label: '任务类型' },
                    { prop: 'F_StartAreaCode', label: '起点库位编码' },
                    { prop: 'F_StartLocCode', label: '起点库区编码' },
                    { prop: 'F_EndAreaCode', label: '终点库区编码' },
                    { prop: 'F_EndLocCode', label: '终点库位编码' },
                    { prop: 'F_GoodsCode', label: '物料类型编号' },
                    { prop: 'F_ContainerCode', label: '容器编码' },
                    { prop: 'F_Quantity', label: '数量' },
                    { prop: 'F_RequestTime', label: '请求发送时间' },
				],
			}
        },
		computed: {},
		...
		methods: {
			...
			handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/agv/ZjnBaseStdPdatask/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
