  ...
  <div class="flow-form"
    ...
    <el-form ref="dataForm"
      ...
      <el-row>
        ...
        <el-col :span="12"
                v-if="judgeShow('leadPeople')">
          <el-form-item label="领料人员"
                        prop="leadPeople">
            <el-input v-model="dataForm.leadPeople"
                      placeholder="入库人"
        ...
        <el-col :span="12"
                v-if="judgeShow('leadDate')">
          <el-form-item label="领料日期"
                        prop="leadDate">
            <el-date-picker v-model="dataForm.leadDate"
                            type="datetime"
                            placeholder="选择日期"
    ...
    <template v-if="judgeShow('entryList')">
      ...
      <el-table :data="dataForm.entryList"
        ...
        <el-table-column prop="materialDemand"
                         label="需数量">
          <template slot-scope="scope">
            <el-input v-model="scope.row.materialDemand"
                      type="number"
                      :disabled="judgeWrite('entryList')"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="proportioning"
                         label="配数量">
          <template slot-scope="scope">
            <el-input v-model="scope.row.proportioning"
...
export default {
  ...
  methods: {
    ...
    count (row) {
      //金额 = 数量*单价
      row.amount = this.hsz.toDecimal(parseFloat(row.price) * parseFloat(row.proportioning))
    },
    handleDel (index) {
