      ...
      <el-row>
        ...
        <el-col :span="6"
          ...
          <el-form-item label="业务人员"
                        prop="salesmanId">
            <user-select v-model="dataForm.salesmanId"
                         placeholder="选择人员"
        ...
        <el-col :span="6"
                v-if="judgeShow('orderCode')">
          <el-form-item label="订单编码">
            <el-input v-model="dataForm.orderCode"
                      placeholder="订单编码"
                      readonly
        ...
        <el-col :span="6"
          ...
          <el-form-item label="付款方式"
                        prop="paymentMode">
            <el-select v-model="dataForm.paymentMode"
                       placeholder="选择付款方式"
        ...
        <el-col :span="6"
                v-if="judgeShow('receivableMoney')">
          <el-form-item label="付款金额">
            <el-input v-model.number="dataForm.receivableMoney"
                      placeholder="付款金额"
        ...
        <el-col :span="6"
                v-if="judgeShow('earnestRate')">
          <el-form-item label="定金比率">
            <el-input v-model="dataForm.earnestRate"
                      placeholder="定金比率"
                      type="number"
        ...
        <el-col :span="6"
                v-if="judgeShow('prepayEarnest')">
          <el-form-item label="预付定金">
            <el-input v-model="dataForm.prepayEarnest"
                      placeholder="预付定金"
                      type="number"
        ...
        <el-col :span="6"
                v-if="judgeShow('deliveryDate')">
          <el-form-item label="发货日期">
            <el-date-picker v-model="dataForm.deliveryDate"
                            type="date"
                            placeholder="选择日期"
                            value-format="timestamp"
                            :editable="false"
        ...
        <el-col :span="12"
                v-if="judgeShow('deliveryAddress')">
    ...
    <div class="box">
      <el-tabs v-model="activeName">
        <el-tab-pane label="订单商品"
          ...
          <el-table :data="dataForm.goodsList"
            ...
            <el-table-column prop="discount"
                             ...
                             width="90">
              <template slot-scope="scope">
                <el-input-number v-model="scope.row.discount"
                                 :precision="2"
                                 :step="0.1"
                                 :min="0"
                                 :max="100"
            ...
            <el-table-column prop="cess"
                             label="税率%"
                             width="90">
              <template slot-scope="scope">
                <el-input-number v-model="scope.row.cess"
                                 :precision="2"
                                 :step="0.1"
                                 :min="0"
                                 :max="100"
            ...
            <el-table-column prop="actualAmount"
                             ...
                             width="80">
              <template slot-scope="scope">
                <el-input v-model="scope.row.actualAmount"
                          readonly
                          :disabled="judgeWrite('goodsList')"></el-input>
              </template>
            </el-table-column>
        ...
        <el-tab-pane label="收款计划"
                     name="plan">
          <el-table :data="dataForm.collectionPlanList"
                    size='mini'
                    show-summary
                    :summary-method="getSummaries"
                    v-if="judgeShow('collectionPlanList')">
            <el-table-column type="index"
                             width="50"
                             label="序号"
                             align="center" />
            <el-table-column prop="receivableDate"
                             label="收款日期">
              <template slot-scope="scope">
                <el-date-picker v-model="scope.row.receivableDate"
                                type="date"
                                value-format="timestamp"
                                :editable="false"
            ...
            <el-table-column prop="receivableRate"
                             label="收款比率%">
              <template slot-scope="scope">
                <el-input-number v-model="scope.row.receivableRate"
                                 :precision="2"
                                 :step="0.1"
                                 :min="0"
                                 :max="100"
            ...
            <el-table-column prop="receivableMoney"
                             label="收款金额">
              <template slot-scope="scope">
            ...
            <el-table-column prop="receivableMode"
                             label="收款方式">
              <template slot-scope="scope">
          ...
          <div class="table-actions"
               ...
               v-if="!setting.readonly && !judgeWrite('collectionPlanList')">
            <el-button type="text"
                       icon="el-icon-plus">新增计划</el-button>
          </div>
        </el-tab-pane>
      </el-tabs>
    </div>
    <GoodsBox v-if="goodsBoxVisible"
              ref="goodsBox"
...
export default {
  ...
  methods: {
    ...
    count (row) {
      //金额 = 数量*单价
      row.amount = this.hsz.toDecimal(parseFloat(row.price) * parseFloat(row.qty))
      //折扣价 = (单价*折扣)
      var discountPrice = row.price * (row.discount / 100);
      //实际单价 = 折扣价 * (1 + (税率 / 100))
      row.actualPrice = this.hsz.toDecimal(discountPrice * (1 + (row.cess / 100)));
      //实际金额
      row.actualAmount = this.hsz.toDecimal(parseFloat(row.actualPrice) * parseFloat(row.qty))
    }
  }
}
</script>
