    ...
    <div class="HSZ-common-layout-center">
      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        ...
        <HSZ-table v-loading="listLoading"
                   :data="list">
          <el-table-column prop="toUserName"
                           label="被委托人"
                           width="150" />
          <el-table-column prop="flowName"
                           label="委托流程"
                           width="250" />
          <el-table-column prop="flowCategory"
                           label="流程分类"
                           width="100">
          ...
          <el-table-column label="状态"
            ...
            <template slot-scope="scope">
              <el-tag type="info"
                      v-if='scope.row.status==1'>未开始</el-tag>
              <el-tag type="danger"
                      v-else-if='scope.row.status==2'>已失效</el-tag>
              <el-tag type="primary"
                      v-else>委托中</el-tag>
            </template>
          </el-table-column>
  ...
  methods: {
    ...
    initData () {
      ...
      FlowDelegateList(query).then(res => {
        ...
        let currTime = this.hsz.toDate(new Date())
        for (let i = 0; i < this.list.length; i++) {
          let e = this.list[i];
          let startTime = this.hsz.toDate(e.startTime);
          let endTime = this.hsz.toDate(e.endTime);
    ...
    formatter (row, column) {
      return this.hsz.dateFormat(row, column)
    },
    // 新增 / 修改
