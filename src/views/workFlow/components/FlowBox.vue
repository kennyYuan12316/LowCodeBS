    ...
    <div class="HSZ-preview-main flow-form-main nohead">
      <div class="btns">
        <el-button type="primary"
                   @click="addComment"
                   v-if="activeTab==='comment'">评 论</el-button>
        <template v-if="(setting.opType=='-1'&&setting.id)||setting.opType==0||setting.opType==1||setting.opType==2">
          <el-button type="primary"
      ...
      <el-tabs class="HSZ-el_tabs"
        ...
        <el-tab-pane label="流转记录"
                     ...
                     v-loading="loading">
          <recordList :list='flowTaskOperatorRecordList'
                      :endTime='endTime' />
        </el-tab-pane>
        <el-tab-pane label="审批汇总"
                     v-if="setting.opType!='-1' && isSummary"
                     v-loading="loading"
                     name="recordSummary">
          <RecordSummary :id='setting.id'
        ...
        <el-tab-pane label="流程评论"
                     ...
                     v-loading="loading"
                     name="comment">
          <Comment :id='setting.id'
                   ref="comment" />
        </el-tab-pane>
      </el-tabs>
      <el-dialog :title="eventType==='audit'?'审批通过':'审批拒绝'"
        ...
        <el-form label-width="130px"
          ...
          <template v-if="eventType==='audit'">
            <el-form-item :label="item.label"
                          ...
                          :key="i"
                          :rules="item.rules">
              <candidate-user-select v-model="item.value"
                                     multiple
          ...
          <el-form-item label="加签人员"
                        v-if="eventType==='audit'&& properties.hasFreeApprover">
            <user-select v-model="handleId"
              ...
              <div class="sign-head">
                ...
                <div class="sign-action">
                  <el-button class="clear-btn"
                             size="mini"
                             @click="handleReset">清空</el-button>
          ...
          <el-form-item label="抄送人员"
                        v-if="properties.isCustomCopy">
            <user-select v-model="copyIds"
                         placeholder="请选择"
                         multiple />
          </el-form-item>
        </el-form>
      ...
      <el-dialog title="指派"
        ...
        <el-form label-width="80px"
                 ...
                 :rules="assignRules"
                 ref="assignForm">
          <el-form-item label="指派节点"
                        prop="nodeCode">
            <el-select v-model="assignForm.nodeCode"
                       placeholder="请选择指派节点">
              <el-option v-for="item in assignNodeList"
                         :key="item.nodeCode"
                         :label="item.nodeName"
                         :value="item.nodeCode" />
            </el-select>
          ...
          <el-form-item label="指派给谁"
            ...
            <user-select v-model="assignForm.freeApproverUserId"
                         placeholder="请选择指派给谁" />
          </el-form-item>
        </el-form>
        <span slot="footer"
          ...
          <el-button type="primary"
                     @click="handleAssign()">{{$t('common.confirmButton')}}
          </el-button>
        </span>
      </el-dialog>
      ...
      <print-browse :visible.sync="printBrowseVisible"
                    :id="properties.printId"
                    :formId="setting.id"
                    :fullName="setting.fullName" />
      <candidate-form :visible.sync="candidateVisible"
          ...
          for (let i = 0; i < this.flowTaskNodeList.length; i++) {
            ...
            const loop = data => {
              ...
              if (data.nodeId === nodeItem.nodeCode) {
                ...
                if (nodeItem.nodeType === 'approver' || nodeItem.nodeType === 'start' || nodeItem.nodeType === 'subFlow' || nodeItem.nodeType === 'dynamic') data.content = nodeItem.userName
                return
              }
