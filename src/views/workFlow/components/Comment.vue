<template>
  <div class="comment-container">
    <div class="comment-list"
      ...
      <template v-if="list.length">
        <div v-for="(item,i) in list"
          ...
          <div class="head">
            <el-avatar :size="40"
                       :src="define.comUrl + item.creatorUserHeadIcon"
                       class="avatar" />
            <p class="username">{{item.creatorUserName}}/{{item.creatorUserId}}</p>
            <el-link :underline="false"
                     class="del-btn"
                     @click="delComment(item.id,i)"
                     type="danger"
          ...
          <div class="content">
            ...
            <div class="img-list"
              ...
              <el-image :src="define.comUrl+cItem.url"
                        ...
                        :key="ci"
                        :preview-src-list="getImgList(item.imageList)"
                        :z-index="10000">
              </el-image>
            </div>
            <div class="file-List"
                 v-if="item.fileList.length">
              <HSZ-UploadFz v-model="item.fileList"
                            detailed
                            disabled></HSZ-UploadFz>
            </div>
          </div>
        </div>
      </template>
    ...
    <el-dialog title="流程评论"
      ...
      <el-form :model="dataForm"
        ...
        <el-form-item>
          <el-upload :action="define.comUploadUrl+'/annexpic'"
            ...
            <el-button size="small"
                       icon="el-icon-picture">图片</el-button>
          </el-upload>
          <el-upload :action="define.comUploadUrl+'/annex'"
                     :headers="uploadHeaders"
                     ref="elUploadFile"
                     ...
                     class="upload-btn">
            <el-button size="small"
                       icon="el-icon-upload">附件</el-button>
          </el-upload>
        </el-form-item>
        <el-form-item prop="image"
                      v-if="dataForm.image.length">
          <div class="img-list">
            <div class="img-item"
                 v-for="(item,i) in dataForm.image"
                 :key="i">
              <el-image :src="define.comUrl+item.url"
                        class=""
                        :preview-src-list="getImgList(dataForm.image)"
                        :z-index="100">
              </el-image>
              <div class="badge"
                   @click.stop="handleImgRemove(i)">
        ...
        <el-form-item prop="file"
                      v-if="dataForm.file.length">
          <transition-group class="el-upload-list el-upload-list el-upload-list--text"
                            tag="ul"
                            name="el-list">
            <li class="el-upload-list__item is-success"
                v-for="(file,index) in dataForm.file"
                :key="file.fileId">
              ...
              <i class="el-icon-view"
                 @click="handleFilePreview(file)"></i>
              <i class="el-icon-download"
                 @click="handleFileClick(file)"></i>
              <label class="el-upload-list__item-status-label">
                <i class="el-icon-upload-success el-icon-circle-check"></i>
              </label>
              <i class="el-icon-close"
                 @click="handleFileRemove(file,index)"></i>
            </li>
          </transition-group>
        </el-form-item>
      </el-form>
    ...
    <Preview :visible.sync="previewVisible"
             :file="activeFile" />
  </div>
</template>
