<template>
  <div class="HSZ-common-layout">
        ...
        <el-form @submit.native.prevent>
          ...
          <el-col :span="6">
            <el-form-item label="所属分类">
              <el-select v-model="flowCategory"
                         placeholder="选择所属分类"
                         clearable>
          ...
          <template v-if="showAll">
            <el-col :span="6">
              <el-form-item label="所属流程">
                <el-select v-model="flowId"
                           placeholder="选择所属流程"
                           clearable>
                  <el-option-group v-for="group in flowEngineList"
                                   :key="group.id"
            ...
            <el-col :span="6">
              <el-form-item label="发起人员">
                <user-select v-model="creatorUserId"
                             placeholder="选择发起人员" />
              </el-form-item>
            </el-col>
          </template>
      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        ...
        <HSZ-table v-loading="listLoading"
          ...
          <el-table-column prop="userName"
                           label="发起人员"
                           width="130" />
          <el-table-column prop="flowUrgent"
                           label="紧急程度"
                           width="130">
          ...
          <el-table-column prop="status"
            ...
            <template slot-scope="scope">
              <el-tag type="success"
                      v-if="scope.row.status==2">审核通过</el-tag>
              <el-tag type="danger"
                      v-else-if="scope.row.status==3">审核驳回</el-tag>
              <el-tag type="warning"
                      v-else-if="scope.row.status==4">流程撤回</el-tag>
              <el-tag type="info"
                      v-else-if="scope.row.status==5">审核终止</el-tag>
              <el-tag type="primary"
                      v-else>等待审核</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              <el-button size="mini"
                         type="text"
                         @click="toDetail(scope.row)">审批</el-button>
            </template>
          </el-table-column>
        </HSZ-table>
    ...
    <FlowBox v-if="formVisible"
             ref="FlowBox"
             @close="closeForm" />
    <BatchList v-if="batchListVisible"
