		...
		<div class="HSZ-common-layout-center">
				...
				<el-form @submit.native.prevent>
					<el-col :span="6">
						<el-form-item label="出库单">
							<el-input v-model="query.F_OutOrder" placeholder="出库单" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list" has-c @selection-change="handleSelectionChange">
					<el-table-column prop="F_OutOrder" label="出库单" align="left" />
					...
					<el-table-column prop="F_OutTimeConfirm" label="出库确认时间" align="left" >
                         <template slot-scope="scope">
                            {{scope.row.F_OutTimeConfirm|toDate("yyyy-MM-dd hh:mm:ss")}}
                        </template>
                    </el-table-column>
					<el-table-column prop="F_OutStation" label="出库站台" align="left" />
					<el-table-column prop="F_OutQuantity" label="出库数量" align="left" />
	    ...
	    data() {
	        ...
	        query: {
                ...
                columnList: [
                    ...
                    { prop: 'F_OutTimeConfirm', label: '出库确认时间' },
                    { prop: 'F_OutStation', label: '出库站台' },
                    { prop: 'F_OutQuantity', label: '出库数量' },
		...
		methods: {
			...
			handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/wms/ZjnReportProductOut/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
            ...
            handleBatchRemoveDel() {
                ...
                this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
                    ...
                    request({
                        url: `/api/wms/ZjnReportProductOut/batchRemove`,
                        method: 'POST',
                        data: ids ,
                    }).then(res => {
                        this.$message({
            ...
            download(data) {
                ...
                request({
                    url: `/api/wms/ZjnReportProductOut/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
