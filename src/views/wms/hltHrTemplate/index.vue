		...
		<div class="HSZ-common-layout-center">
				...
				<el-form @submit.native.prevent>
					<el-col :span="6">
						<el-form-item label="系统名称">
							<el-input v-model="query.g2" placeholder="系统名称" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="系统模板名称">
							<el-input v-model="query.g3" placeholder="系统模板名称" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item>
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list" has-c @selection-change="handleSelectionChange">
					<el-table-column prop="g1" label="编号" align="left" />
					<el-table-column prop="g2" label="系统名称" align="left" />
					<el-table-column prop="g3" label="系统模板名称" align="left" />
					<el-table-column prop="g4" label="调用类型值" align="left" />
					<el-table-column prop="g5" label="超时(毫秒)" align="left" />
					<el-table-column prop="g6" label="超时重派次数" align="left" />
					<el-table-column prop="g7" label="绕过SSL认证" align="left" />
					<el-table-column prop="g8" label="单次发送的数据总量" align="left" />
	    ...
	    data() {
	        ...
	        query: {
                ...
                columnList: [
                    { prop: 'g1', label: '编号' },
                    { prop: 'g2', label: '系统名称' },
                    { prop: 'g3', label: '系统模板名称' },
                    { prop: 'g4', label: '调用类型值' },
                    { prop: 'g5', label: '超时(毫秒)' },
                    { prop: 'g6', label: '超时重派次数' },
                    { prop: 'g7', label: '绕过SSL认证' },
                    { prop: 'g8', label: '单次发送的数据总量' },
				],
			}
        },
		computed: {},
		...
		methods: {
            ...
            handleBatchRemoveDel() {
                ...
                this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
                    ...
                    request({
                        url: `/api/wms/HltHrTemplate/batchRemove`,
                        method: 'POST',
                        data: ids ,
                    }).then(res => {
                        this.$message({
            ...
            download(data) {
                ...
                request({
                    url: `/api/wms/HltHrTemplate/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
