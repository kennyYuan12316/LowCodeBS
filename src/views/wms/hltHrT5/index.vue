		...
		<div class="HSZ-common-layout-center">
			<el-row class="HSZ-common-search-box" :gutter="16">
				<el-form @submit.native.prevent>
					<el-col :span="6">
						<el-form-item label="接收类型名称">
							<el-input v-model="query.t2" placeholder="接收类型名称" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="接收类型编码">
							<el-input v-model="query.t3" placeholder="接收类型编码" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item>
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list" has-c @selection-change="handleSelectionChange">
					<el-table-column prop="t1" label="编号" align="left" />
					<el-table-column prop="t2" label="接收类型名称" align="left" />
					<el-table-column prop="t3" label="接收类型编码" align="left" />
					<el-table-column prop="t4" label="接收类型原始编码" align="left" />
					<el-table-column prop="t5" label="接收类型转换编码" align="left" />
	...
	export default {
	    ...
	    data() {
	        ...
	        query: {
                ...
                columnList: [
                    { prop: 't1', label: '编号' },
                    { prop: 't2', label: '接收类型名称' },
                    { prop: 't3', label: '接收类型编码' },
                    { prop: 't4', label: '接收类型原始编码' },
                    { prop: 't5', label: '接收类型转换编码' },
				],
			}
        },
		computed: {},
		...
		methods: {
            ...
            handleBatchRemoveDel() {
                ...
                this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
                    ...
                    request({
                        url: `/api/wms/HltHrT5/batchRemove`,
                        method: 'POST',
                        data: ids ,
                    }).then(res => {
                        this.$message({
            ...
            download(data) {
                ...
                request({
                    url: `/api/wms/HltHrT5/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
