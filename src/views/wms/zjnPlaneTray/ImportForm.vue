  ...
  <el-dialog title="批量导入" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <div class="import-main" v-show="active==2">
      <HSZ-table v-loading="listLoading" :data="list">
        <el-table-column prop="F_TrayNo" label="托盘编号" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_TrayNo" />
          </template>
        </el-table-column>
        <el-table-column prop="TypeNmae" label="托盘类型" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.TypeNmae" />
          </template>
        </el-table-column>
        <el-table-column prop="TrayStateName" label="托盘状态" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.TrayStateName" />
          </template>
        </el-table-column>
    ...
    methods: {
      ...
      next() {
        if (this.active == 2) {
          ...
          console.log(datas)
          request({
            url: `/api/wms/ZjnPlaneTray/RawMaterialWarehousingData`,
            method: 'POST',
            data: this.list
          }).then(res => {
        ...
        if (this.active == 1) {
          ...
          request({
            url: `/api/wms/ZjnPlaneTray/ImportPreview`,
            method: 'GET',
            data: datas
          }).then(res => {
            if(res.data.dataRow.length > 0 && res.data.dataRow[0].F_DisableMark == "案例不会导入"){
      ...
      templateDownload() {
        ...
        request({
          url: `/api/wms/ZjnPlaneTray/TheRawMaterial`,
          method: 'GET',
          data: query
        }).then(res => {
      ...
      exportExceptionData() {
        ...
        //导出错误信息
        request({
          url: `/api/wms/ZjnPlaneTray/ExportExceptionData`,
          method: 'POST',
