    ...
    <el-dialog title="批量导入" :close-on-click-modal="false" :visible.sync="visible"
      ...
      <div class="import-main" v-show="active==2">
        <HSZ-table v-loading="listLoading" :data="list">
          ...
          <el-table-column prop="AisleNo" label="巷道编号" >
            <template slot-scope="scope">
              <el-input v-model="scope.row.AisleNo" />
            </template>
          </el-table-column>
      ...
      methods: {
        ...
        next() {
          if (this.active == 2) {
            ...
            console.log(datas)
            request({
              url: `/api/wms/ZjnWmsLocation/RawMaterialWarehousingData`,
              method: 'POST',
              data: this.list
            }).then(res => {
          ...
          if (this.active == 1) {
            ...
            request({
              url: `/api/wms/ZjnWmsLocation/ImportPreview`,
              method: 'GET',
              data: datas
            }).then(res => {
        ...
        templateDownload() {
          ...
          request({
            url: `/api/wms/ZjnWmsLocation/TheRawMaterial`,
            method: 'GET',
            data: query
          }).then(res => {
        ...
        exportExceptionData() {
          ...
          //导出错误信息
          request({
            url: `/api/wms/ZjnWmsLocation/ExportExceptionData`,
            method: 'POST',
