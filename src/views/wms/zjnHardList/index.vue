        ...
        <el-form @submit.native.prevent>
          <el-col :span="6">
            <el-form-item label="状态">
              <el-input v-model="query.F_Status"
                        placeholder="状态"
                        clearable />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="类型">
              <el-select v-model="query.F_Type"
                         placeholder="类型"
                         clearable>
          ...
          <el-col :span="6">
            <el-form-item label="设备名称">
              <el-input v-model="query.F_HardName"
                        placeholder="设备名称"
                        clearable />
            </el-form-item>
          </el-col>
          <template v-if="showAll">
            <el-col :span="6">
              <el-form-item label="设备编号">
                <el-input v-model="query.F_HardNo"
                          placeholder="设备编号"
                          clearable />
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="虚拟设备">
                <el-select v-model="query.F_FictitiousHard"
                           placeholder="虚拟设备">
                  <el-option v-for="(item, index) in fictitiousHardOptions"
                             :key="index"
                             :label="item.fullName"
                             :value="item.id" />
                </el-select>
      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        <div class="HSZ-common-head">
          <div>
            ...
            <el-button type="text"
                       icon="el-icon-delete"
                       @click="handleBatchRemoveDel()">批量删除</el-button>
          </div>
        ...
        <HSZ-table v-loading="listLoading"
          ...
          <el-table-column prop="F_HardNo"
                           label="设备编号"
                           align="left" />
          <el-table-column prop="F_HardName"
                           label="设备名称"
                           align="left" />
          <el-table-column label="类型"
                           prop="type"
                           align="left">
            <template slot-scope="scope">{{ scope.row.F_Type | dynamicText(typeOptions) }}</template>
          </el-table-column>
          <el-table-column label="虚拟设备"
                           prop="fictitiousHard"
                           align="left">
            <template slot-scope="scope">{{ scope.row.F_FictitiousHard | dynamicText(fictitiousHardOptions) }}</template>
          </el-table-column>
          <el-table-column prop="F_CreateUser"
                           label="创建者"
                           align="left" />
          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              ...
              <el-button type="text"
                         @click="goDetail(scope.row.F_Id)">详情</el-button>
            </template>
          </el-table-column>
        </HSZ-table>
  ...
  data () {
    return {
      ...
      query: {
        F_Status: undefined,
        F_Type: undefined,
        F_HardName: undefined,
        F_HardNo: undefined,
        F_FictitiousHard: undefined,
      },
      list: [],
      listLoading: true,
      multipleSelection: [],
      ...
      columnList: [
        { prop: 'F_HardNo', label: '设备编号' },
        { prop: 'F_HardName', label: '设备名称' },
        { prop: 'F_Type', label: '类型' },
        { prop: 'F_FictitiousHard', label: '虚拟设备' },
  ...
  methods: {
    ...
    handleBatchRemoveDel () {
      ...
      this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
        ...
        request({
          url: `/api/wms/ZjnHardList/batchRemove`,
          method: 'POST',
          data: ids,
        }).then(res => {
          this.$message({
    ...
    download (data) {
      ...
      request({
        url: `/api/wms/ZjnHardList/Actions/Export`,
        method: 'GET',
        data: query
      }).then(res => {
