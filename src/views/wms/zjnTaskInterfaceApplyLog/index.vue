    ...
    <div class="HSZ-common-layout-center">
        ...
        <el-form @submit.native.prevent>
          <el-col :span="6">
            <el-form-item label="接口名">
              <el-input v-model="query.F_InterfaceName" placeholder="接口名" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="6">
      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        ...
        <HSZ-table v-loading="listLoading" :data="list" has-c @selection-change="handleSelectionChange">
          <el-table-column prop="F_InterfaceName" label="接口名" align="left" />
          <el-table-column prop="F_Address" label="调用完整地址" align="left" />
          <el-table-column prop="F_EnterParameter" label="入参" align="left" />
          <el-table-column prop="F_OutParameter" label="出参" align="left" />
          <el-table-column prop="F_Msg" label="消息" align="left" />
          <el-table-column prop="F_CreateTime" label="调用时间" align="left">
...
export default {
      ...
      columnList: [
        { prop: 'F_InterfaceName', label: '接口名' },
        { prop: 'F_Address', label: '调用完整地址' },
        { prop: 'F_EnterParameter', label: '入参' },
        { prop: 'F_OutParameter', label: '出参' },
        { prop: 'F_Msg', label: '消息' },
        { prop: 'F_CreateTime', label: '调用时间' }
      ]
    }
  },
  computed: {},
  ...
  methods: {
    ...
    handleDel(id) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        request({
          url: `/api/wms/ZjnTaskInterfaceApplyLog/${id}`,
          method: 'DELETE'
        }).then(res => {
          this.$message({
            type: 'success',
    ...
    handleBatchRemoveDel() {
      ...
      this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
        ...
        request({
          url: `/api/wms/ZjnTaskInterfaceApplyLog/batchRemove`,
          method: 'POST',
          data: ids
        }).then(res => {
          this.$message({
    ...
    download(data) {
      ...
      request({
        url: `/api/wms/ZjnTaskInterfaceApplyLog/Actions/Export`,
        method: 'GET',
        data: query
      }).then(res => {
