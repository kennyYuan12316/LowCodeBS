		...
		<div class="HSZ-common-layout-center">
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list" has-c @selection-change="handleSelectionChange">
					...
					<el-table-column prop="F_LocationAge" label="库龄" align="left" />
					...
					<el-table-column prop="F_BatteryType" label="电池型号" align="left" />
					<el-table-column prop="F_BatteryCount" label="电池数量" align="left" />
	...
	export default {
	    ...
	    data() {
	        ...
	        query: {
                ...
                columnList: [
                    ...
                    { prop: 'F_LocationAge', label: '库龄' },
                    { prop: 'F_CombineType', label: '集成方式' },
                    { prop: 'F_ProjectNum', label: '项目号' },
                    { prop: 'F_PackingType', label: '包装方式' },
                    { prop: 'F_BatteryType', label: '电池型号' },
                    { prop: 'F_BatteryCount', label: '电池数量' },
		...
		methods: {
			initData() {
                ...
                request({
                    url: `/api/wms/ZjnReportProductstructurePack`,
                    method: 'GET',
                    data: query
                }).then(res => {
			...
			handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/wms/ZjnReportProductstructurePack/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
            ...
            handleBatchRemoveDel() {
                ...
                this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
                    ...
                    request({
                        url: `/api/wms/ZjnReportProductstructurePack/batchRemove`,
                        method: 'POST',
                        data: ids ,
                    }).then(res => {
                        this.$message({
            ...
            download(data) {
                ...
                request({
                    url: `/api/wms/ZjnReportProductstructurePack/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
