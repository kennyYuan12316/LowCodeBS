		...
		<div class="HSZ-common-layout-center">
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list">
					<el-table-column label="PLC连接" prop="plcConnectionId" align="left">
					...
					<el-table-column prop="F_ObjectType" label="对像" align="left" />
					<el-table-column prop="F_RequestServices" label="请求服务" align="left" />
	...
	export default {
	    ...
	    data() {
	        ...
	        query: {
                ...
                columnList: [
                    { prop: 'F_PlcConnectionId', label: 'PLC连接' },
                    { prop: 'F_Address', label: '地址' },
                    { prop: 'F_Offset', label: '偏移量' },
                    { prop: 'F_Length', label: '长度' },
                    { prop: 'F_DeviceNo', label: '设备号' },
                    { prop: 'F_ObjectType', label: '对像' },
                    { prop: 'F_RequestServices', label: '请求服务' },
		...
		methods: {
			...
			handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/wms/ZjnPlcReadRequest/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
            ...
            download(data) {
                ...
                request({
                    url: `/api/wms/ZjnPlcReadRequest/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
