		...
		<div class="HSZ-common-layout-center">
			<el-row class="HSZ-common-search-box" :gutter="16">
				<el-form @submit.native.prevent>
					<el-col :span="6">
						<el-form-item label="所属托盘">
							<el-input v-model="query.F_Tray" placeholder="所属托盘" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="电芯条码">
							<el-input v-model="query.F_BatteryCode" placeholder="电芯条码" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
					...
					<el-col :span="6">
						<el-form-item label="入库单">
							<el-input v-model="query.F_EntryOrder" placeholder="入库单" clearable />	
						</el-form-item>
					</el-col>
					</template>
					<el-col :span="6">
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list" has-c @selection-change="handleSelectionChange">
					<el-table-column prop="F_Tray" label="所属托盘" align="left" />
					<el-table-column prop="F_BatteryCode" label="电芯条码" align="left" />
					<el-table-column prop="F_ProductionOrder" label="生产单号" align="left" />
					...
					<el-table-column prop="F_EntryOrder" label="入库单" align="left" />
					<el-table-column prop="F_EntryTimeFitrst" label="首次入库时间" align="left">
                         <template slot-scope="scope">
                            {{scope.row.F_EntryTimeFitrst|toDate("yyyy-MM-dd hh:mm:ss")}}
                        </template>
                    </el-table-column>
					...
					<el-table-column prop="F_ProductionMark" label="产品标识" align="left" />
					<el-table-column prop="F_InventoryMark" label="库存标识" align="left" />
					<el-table-column prop="F_ClassMark" label="等级标识" align="left" />
					<el-table-column prop="F_Voltage" label="电压" align="left" />
					<el-table-column prop="F_Ah" label="安时" align="left" />
					<el-table-column prop="F_ACR" label="交流电阻" align="left" />
					<el-table-column prop="F_DCR" label="直流电阻" align="left" />
					<el-table-column prop="F_KValue" label="K值" align="left" />
					<el-table-column prop="F_ProductionTime" label="生产时间" align="left">
                         <template slot-scope="scope">
                            {{scope.row.F_ProductionTime|toDate("yyyy-MM-dd hh:mm:ss")}}
                        </template>
                    </el-table-column>
					<el-table-column prop="F_LineNum" label="生产线" align="left" />
					<el-table-column prop="F_LineStatus" label="生产线状态" align="left" />
					<el-table-column prop="F_ChannelNum" label="通道号" align="left" />
					<el-table-column prop="F_Location" label="位置" align="left" />
					<el-table-column prop="F_Freeze" label="是否冻结" align="left" />
					<el-table-column prop="F_FreezeResult" label="冻结原因" align="left" />
...
<script>
	...
	export default {
	    ...
	    data() {
	        ...
	        query: {
	                F_Tray:undefined,
	                F_BatteryCode:undefined,
	                F_Batch:undefined,
	                F_EntryOrder:undefined,
				},
                list: [],
                listLoading: true,
                multipleSelection: [], 
                ...
                columnList: [
                    { prop: 'F_Tray', label: '所属托盘' },
                    { prop: 'F_BatteryCode', label: '电芯条码' },
                    { prop: 'F_ProductionOrder', label: '生产单号' },
                    { prop: 'F_ProductsCode', label: '物料编码' },
                    { prop: 'F_ProductsName', label: '物料名称' },
                    { prop: 'F_Batch', label: '32位批次号' },
                    { prop: 'F_EntryOrder', label: '入库单' },
                    { prop: 'F_ComboTime', label: '组盘时间' },
                    { prop: 'F_EntryTimeFitrst', label: '首次入库时间' },
                    { prop: 'F_EntryTime', label: '入库时间' },
                    { prop: 'F_ProductsQuantity', label: '物料数量' },
                    { prop: 'F_ProductsUnit', label: '物料单位' },
                    { prop: 'F_Quality', label: '质量状态' },
                    { prop: 'F_ProductionMark', label: '产品标识' },
                    { prop: 'F_InventoryMark', label: '库存标识' },
                    { prop: 'F_ClassMark', label: '等级标识' },
                    { prop: 'F_Voltage', label: '电压' },
                    { prop: 'F_Ah', label: '安时' },
                    { prop: 'F_ACR', label: '交流电阻' },
                    { prop: 'F_DCR', label: '直流电阻' },
                    { prop: 'F_KValue', label: 'K值' },
                    { prop: 'F_ProductionTime', label: '生产时间' },
                    { prop: 'F_LineNum', label: '生产线' },
                    { prop: 'F_LineStatus', label: '生产线状态' },
                    { prop: 'F_ChannelNum', label: '通道号' },
                    { prop: 'F_Location', label: '位置' },
                    { prop: 'F_Freeze', label: '是否冻结' },
                    { prop: 'F_FreezeResult', label: '冻结原因' },
				],
			}
        },
		computed: {},
		...
		methods: {
			...
			handleDel(id) {
                this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
                    ...
                    request({
                        url: `/api/wms/ZjnReportProductSingle/${id}`,
                        method: 'DELETE'
                    }).then(res => {
                        this.$message({
                            type: 'success',
            ...
            handleBatchRemoveDel() {
                ...
                this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
                    ...
                    request({
                        url: `/api/wms/ZjnReportProductSingle/batchRemove`,
                        method: 'POST',
                        data: ids ,
                    }).then(res => {
                        this.$message({
            ...
            download(data) {
                ...
                request({
                    url: `/api/wms/ZjnReportProductSingle/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
