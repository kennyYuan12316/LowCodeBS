  ...
  >
    ...
    <div v-show="active==2" class="import-main">
      <HSZ-table v-loading="listLoading" :data="list">
        ...
        <el-table-column prop="PurchaseOrder" label="单据号">
          <template slot-scope="scope">
        ...
        <el-table-column prop="ExpiryDate" label="失效日期">
          ...
          <template slot-scope="scope">
            <el-input v-model="scope.row.ExpiryDate" placeholder="请选择" clearable :style="{&quot;width&quot;:&quot;100%&quot;}" type="datetime" format="yyyy-MM-dd HH:mm:ss" value-format="timestamp" />
          </template>
        </el-table-column>

        <el-table-column prop="CreateUser" label="检验状态">
          <template slot-scope="scope">
            <el-input v-model="scope.row.CreateUser" />
          </template>
        </el-table-column>
        ...
        <el-table-column prop="ProductsUser" label="客户编号">
          <template slot-scope="scope">
            <el-input v-model="scope.row.ProductsUser" />
          </template>
        </el-table-column>

        <el-table-column prop="TheTray" label="普通托盘号">
          <template slot-scope="scope">
            <el-input v-model="scope.row.TheTray" />
          </template>
        </el-table-column>

        <el-table-column prop="TheContainer" label="容器托盘号">
          <template slot-scope="scope">
            <el-input v-model="scope.row.TheContainer" />
          </template>
        </el-table-column>
    ...
    <div v-show="active==3" class="import-main">
      ...
      <div v-if="result.resultType" class="unsuccess">
        ...
        <HSZ-table :data="resultList" height="280px">
          <el-table-column prop="PurchaseOrder" label="单据号" />
          <el-table-column prop="OrderNo" label="32位流水号" />
          <el-table-column prop="ProductsTotal" label="物料数量" />
          <el-table-column prop="ProductsLocation" label="物料货位" />
          <el-table-column prop="ExpiryDate" label="失效日期" />
          <el-table-column prop="CreateUser" label="检验状态" />
  ...
  methods: {
    ...
    next() {
      if (this.active == 2) {
        ...
        request({
          url: `/api/wms/ZjnBillsHistory/RawMaterialWJgJingData`,
          method: 'POST',
          data: this.list
        }).then(res => {
