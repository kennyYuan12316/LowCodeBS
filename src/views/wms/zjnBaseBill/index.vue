    ...
    <div class="HSZ-common-layout-center">
        ...
        <el-form @submit.native.prevent>
          <el-col :span="6">
            <el-form-item label="单据编号">
              <el-input v-model="query.F_BillNo"
                        placeholder="单据编号"
                        clearable />
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="类型">
              <num-range v-model="query.F_Type"></num-range>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="有效标志">
              <el-select v-model="query.F_EnabledMark"
                         placeholder="有效标志">
                <el-option v-for="(item, index) in enabledMarkOptions"
                           :key="index"
                           :label="item.fullName"
                           :value="item.id" />
              </el-select>
      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        ...
        <HSZ-table v-loading="listLoading"
          ...
          <el-table-column prop="F_BillNo"
                           label="单据编号"
                           align="left" />
          <el-table-column prop="F_BillName"
                           label="单据名称"
                           align="left" />
          <el-table-column prop="F_Type"
                           label="类型"
                           align="left" />
          <el-table-column prop="F_CreateUser"
                           label="创建者"
                           align="left" />
          <el-table-column prop="F_CreateTime"
                           label="创建时间"
                           align="left" />
...
export default {
  ...
  data () {
    return {
      ...
      columnList: [
        { prop: 'F_BillNo', label: '单据编号' },
        { prop: 'F_BillName', label: '单据名称' },
  ...
  methods: {
    ...
    handleBatchRemoveDel () {
      ...
      this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
        ...
        request({
          url: `/api/wms/ZjnBaseBill/batchRemove`,
          method: 'POST',
          data: ids,
        }).then(res => {
          this.$message({
    ...
    download (data) {
      ...
      request({
        url: `/api/wms/ZjnBaseBill/Actions/Export`,
        method: 'GET',
        data: query
      }).then(res => {
