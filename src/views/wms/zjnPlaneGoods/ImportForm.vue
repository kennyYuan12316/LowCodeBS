  ...
  <el-dialog title="批量导入" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <div class="import-main" v-show="active==2">
      <HSZ-table v-loading="listLoading" :data="list">
        <el-table-column prop="F_GoodsCode" label="物料编码" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_GoodsCode" />
          </template>
        </el-table-column>
        <el-table-column prop="F_GoodsName" label="物料名称" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_GoodsName" />
          </template>
        </el-table-column>
        <el-table-column prop="GoodsTypeName" label="物料类型" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.GoodsTypeName" />
          </template>
        </el-table-column>
        ...
        <el-table-column prop="GoodsStateName" label="物料状态" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.GoodsStateName" />
          </template>
        </el-table-column>
        <el-table-column prop="CheckTypeNmae" label="校验类型" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.CheckTypeNmae" />
          </template>
        </el-table-column>
        <el-table-column prop="F_Ceiling" label="库存上限" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_Ceiling" />
          </template>
        </el-table-column>
        <el-table-column prop="F_TheLowerLimit" label="库存下限" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_TheLowerLimit" />
          </template>
        </el-table-column>
        <el-table-column prop="F_Specifications" label="物料规格" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_Specifications" />
          </template>
        </el-table-column>
        <el-table-column prop="F_CustomerId" label="客户编码" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_CustomerId" />
          </template>
        </el-table-column>

         <el-table-column prop="F_VendorId" label="供应商编码" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_VendorId" />
          </template>
        </el-table-column>
        <el-table-column prop="F_IsFirstOut" label="先入先出" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_IsFirstOut" />
          </template>
        </el-table-column>
        <el-table-column prop="F_TellDate" label="预警周期" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_TellDate" />
          </template>
        </el-table-column>
        <el-table-column prop="F_DisableMark" label="禁用原因" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_DisableMark" />
          </template>
        </el-table-column>
    ...
    <div class="import-main" v-show="active==3">
      ...
      <div class="unsuccess" v-if="result.resultType">
        ...
        <HSZ-table :data="resultList" height="280px">
          ...
          <el-table-column prop="F_TheLowerLimit" label="库存下限" />
          <el-table-column prop="F_Specifications" label="物料规格" />
          <el-table-column prop="F_CustomerId" label="客户编号"/>
          <el-table-column prop="F_VendorId" label="供应商编号" />
          <el-table-column prop="CheckTypeNmae" label="检验类型"/>
          <el-table-column prop="F_IsFirstOut" label="先入先出(1是 0否)" />
          <el-table-column prop="F_TellDate" label="预警周期(天)"/>
          <el-table-column prop="F_DisableMark" label="禁用原因" />
        </HSZ-table>
      </div>
    </div>
    ...
    methods: {
      ...
      next() {
        if (this.active == 2) {
          ...
          console.log(datas)
          request({
            url: `/api/wms/ZjnPlaneGoods/RawMaterialWarehousingData`,
            method: 'POST',
            data: this.list
          }).then(res => {
        ...
        if (this.active == 1) {
          ...
          request({
            url: `/api/wms/ZjnPlaneGoods/ImportPreview`,
            method: 'GET',
            data: datas
          }).then(res => {
            if(res.data.dataRow.length > 0 && res.data.dataRow[0].F_GoodsName == "案例"){
      ...
      templateDownload() {
        ...
        request({
          url: `/api/wms/ZjnPlaneGoods/TheRawMaterial`,
          method: 'GET',
          data: query
        }).then(res => {
      ...
      exportExceptionData() {
        let query = {
          ...
          fileName:"物料基础入库记录失败记录"
        }
        //导出错误信息
        request({
          url: `/api/wms/ZjnPlaneGoods/ExportExceptionData`,
          method: 'POST',
