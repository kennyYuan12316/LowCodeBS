				...
				<el-form ref="elForm" :model="dataForm" size="small" label-width="100px" label-position="right" :disabled="!!isDetail" :rules="rules">
					...
					<el-col :span="24">
						<el-form-item label="供应商编号" prop="supplierNo">
							<el-input :disabled = "istrues" v-model='dataForm.supplierNo' placeholder='请输入供应商编号' clearable required :style='{"width":"100%"}' >
							</el-input>
						</el-form-item>
					</el-col>
					<el-col :span="24">
...
<script>
	...
	export default {
		...
		props: [],
		data() {
			var checksupplierNo = (rule, value, callback) => {
      if (!value) {
        callback(new Error('请输入供应商编号'))
      } else if(!this.dataForm.id) {

          request({
            url: `/api/wms/BaseSupplier/ExistSupplierNo`,
            method: 'GET',
            data: { SupplierNo: value }
          }).then((res) => {
            if (res.data) {
		...
		methods: {
			...
			init(id, isDetail) {
				...
				this.$nextTick(() => {
					...
					if (this.dataForm.id) {
						request({
							url: '/api/wms/BaseSupplier/' + this.dataForm.id,
							method: 'get'
						}).then(res =>{
