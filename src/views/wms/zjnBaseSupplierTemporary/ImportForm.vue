    ...
    methods: {
      ...
      next() {
        if (this.active == 2) {
          ...
          console.log(datas)
          request({
            url: `/api/wms/BaseSupplier/ImportData`,
            method: 'POST',
            data: this.list
          }).then(res => {
        ...
        if (this.active == 1) {
          ...
          request({
            url: `/api/wms/BaseSupplier/ImportPreview`,
            method: 'GET',
            data: datas
          }).then(res => {
            if(res.data.dataRow.length > 0 && res.data.dataRow[0].supplierName == "案例"){
      ...
      templateDownload() {
        request({
          url: `/api/wms/BaseSupplier/CreateModel`,
          method: 'GET',
          data: null
        }).then(res => {
      ...
      exportExceptionData() {
        //导出错误信息
        request({
          url: `/api/wms/BaseSupplier/ExportExceptionData`,
          method: 'POST',
