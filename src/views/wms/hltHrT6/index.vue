		...
		<div class="HSZ-common-layout-center">
				...
				<el-form @submit.native.prevent>
					<el-col :span="6">
						<el-form-item label="接收类型">
							<el-input v-model="query.t2" placeholder="接收类型" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="接收属性模板名称">
							<el-input v-model="query.t3" placeholder="接收属性模板名称" clearable />	
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item>
			...
			<div class="HSZ-common-layout-main HSZ-flex-main">
                ...
                <HSZ-table v-loading="listLoading" :data="list" has-c @selection-change="handleSelectionChange">
					...
					<el-table-column prop="t2" label="接收类型" align="left" />
					<el-table-column prop="t3" label="接收属性模板名称" align="left" />
					<el-table-column prop="t4" label="接收属性模板原始编码" align="left" />
					<el-table-column prop="t5" label="接收属性模板转换编码" align="left" />
	    ...
	    data() {
	        ...
	        query: {
                ...
                columnList: [
                    { prop: 't1', label: '编号' },
                    { prop: 't2', label: '接收类型' },
                    { prop: 't3', label: '接收属性模板名称' },
                    { prop: 't4', label: '接收属性模板原始编码' },
                    { prop: 't5', label: '接收属性模板转换编码' },
				],
			}
        },
		computed: {},
		...
		methods: {
            ...
            handleBatchRemoveDel() {
                ...
                this.$confirm('您确定要删除这些数据吗, 是否继续？', '提示', {
                    ...
                    request({
                        url: `/api/wms/HltHrT6/batchRemove`,
                        method: 'POST',
                        data: ids ,
                    }).then(res => {
                        this.$message({
            ...
            download(data) {
                ...
                request({
                    url: `/api/wms/HltHrT6/Actions/Export`,
                    method: 'GET',
                    data: query
                }).then(res => {
