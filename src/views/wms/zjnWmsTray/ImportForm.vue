  ...
  <el-dialog title="批量导入" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <div class="import-main" v-show="active==2">
      <HSZ-table v-loading="listLoading" :data="list">
        ...
        <el-table-column prop="F_TrayName" label="托盘名称" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_TrayName" />
          </template>
        </el-table-column>
        <el-table-column prop="F_TypeName" label="托盘类型" >
          <template slot-scope="scope">
            <el-input v-model="scope.row.F_TypeName" />
          </template>
        </el-table-column>
        <el-table-column prop="EnabledMark" label="禁用原因" >
          <template slot-scope="scope">
    ...
    methods: {
      ...
      next() {
        if (this.active == 2) {
          ...
          console.log(datas)
          request({
            url: `/api/wms/ZjnWmsTray/RawMaterialWarehousingData`,
            method: 'POST',
            data: this.list
          }).then(res => {
        ...
        if (this.active == 1) {
          ...
          request({
            url: `/api/wms/ZjnWmsTray/ImportPreview`,
            method: 'GET',
            data: datas
          }).then(res => {
            if(res.data.dataRow.length > 0 && res.data.dataRow[0].EnabledMark == "案例不会导入"){
      ...
      templateDownload() {
        ...
        request({
          url: `/api/wms/ZjnWmsTray/CreateModel`,
          method: 'GET',
          data: query
        }).then(res => {
      ...
      exportExceptionData() {
        ...
        //导出错误信息
        request({
          url: `/api/wms/ZjnWmsTray/ExportExceptionData`,
          method: 'POST',
