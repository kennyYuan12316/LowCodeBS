      ...
      <el-scrollbar class="layout-area">
        <grid-layout :layout.sync="layout"
          ...
          <grid-item v-for="item in layout"
            ...
            <Todo v-if="item.hszKey==='todo'"
                  :title="item.title" />
            <CommonFunc v-if="item.hszKey==='commonFunc'"
                        :title="item.title"
                        :list="item.list" />
            <TodoList v-if="item.hszKey==='todoList'"
                      :title="item.title" />
            <HNotice v-if="item.hszKey==='notice'"
                     :title="item.title" />
            <HEmail v-if="item.hszKey==='email'"
                    :title="item.title" />
            <DataBoard v-if="item.hszKey==='dataBoard'"
                       :title="item.title"
                       :list="item.list" />
            <HBarChart v-if="item.hszKey==='barChart'"
                       :title="item.title"
                       :option="item.option"
                       :dataType="item.dataType"
                       :propsApi="item.propsApi"
                       :ref="'eChart'+item.i" />
            <HAnnularChart v-if="item.hszKey==='annularChart'"
                           :title="item.title"
                           :option="item.option"
                           :dataType="item.dataType"
                           :propsApi="item.propsApi"
                           :ref="'eChart'+item.i" />
            <HAreaChart v-if="item.hszKey==='areaChart'"
                        :title="item.title"
                        :option="item.option"
                        :dataType="item.dataType"
                        :propsApi="item.propsApi"
                        :ref="'eChart'+item.i" />
            <HLineChart v-if="item.hszKey==='lineChart'"
                        :title="item.title"
                        :option="item.option"
                        :dataType="item.dataType"
                        :propsApi="item.propsApi"
            ...
            <HPieChart v-if="item.hszKey==='pieChart'"
                       :title="item.title"
                       :option="item.option"
                       :dataType="item.dataType"
                       :propsApi="item.propsApi"
                       :ref="'eChart'+item.i" />
            <HRadarChart v-if="item.hszKey==='radarChart'"
                         :title="item.title"
                         :option="item.option"
                         :dataType="item.dataType"
                         :propsApi="item.propsApi"
        ...
        <div v-show="!layout.length"
             class="empty-info">
          <img src="@/assets/images/emptyPortal.png"
               alt=""
               class="empty-img">
        </div>
      </el-scrollbar>
