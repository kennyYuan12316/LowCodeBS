<template>
  <el-aside width="300px"
    ...
    <el-scrollbar class="aside-scrollbar">
      <el-form size="small"
        ...
        <template v-if="activeData">
          <el-form-item v-if="activeData.title !== undefined"
                        label="标题">
            <el-input v-model="activeData.title"
                      placeholder="请输入标题" />
          </el-form-item>
          <template v-if="activeData.hszKey === 'commonFunc'">
            <el-divider>选项</el-divider>
            <div v-for="(item, index) in activeData.list"
                 :key="index"
                 class="select-item">
              <HSZ-TreeSelect :options="menuList"
                              v-model="item.id"
                              placeholder="请选择菜单"
                              lastLevel
                              lastLevelKey='type'
          ...
          <template v-if="activeData.hszKey === 'dataBoard'">
            <div v-for="(item, index) in activeData.list"
                 :key="index"
                 class="dataBoard-item">
              <p class="head"><span>选项{{index+1}}</span>
                <a class="head-icon el-icon-close"
                   @click="delDataBoardItem(index)"></a>
              </p>
              ...
              <el-form-item label="数据接口"
                            v-if="item.dataType==='dynamic'">
                <HSZ-TreeSelect :options="dataInterfaceOptions"
          ...
          <template v-if="activeData.hszKey && activeData.hszKey.indexOf('Chart')>-1">
            <el-form-item label="数据类型">
    ...
    <JSONArea v-if="areaVisible"
              ref="JSONArea"
              @refresh="updateOption" />
    <iconBox :visible.sync="iconsVisible"
             :current="current"
             @choiceIcon="setIcon" />
  </el-aside>
</template>
