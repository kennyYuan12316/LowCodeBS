    ...
    <div class="print-template-detail__body">
      <div class="system-view-nav">
        <el-scrollbar class="left-scrollbar">
          ...
          <el-tree :data="newBarCodeData"
                   default-expand-all
                   :expand-on-click-node="false"
                   :props="defaultProps"
                   @node-click="createQRcode"></el-tree>
        </el-scrollbar>
      </div>
      <div class="system-view-content">
        <ts-designer-tinymce v-model="content"
                             ref="createTinymce"
                             :height="richHeight"
                             :init="getEditConfig()"
                             class="rich-txt" />
...
export default {
  ...
  computed: {
    ...
    newBarCodeData () {
      return this.newBarCodeData
    }
  },
  methods: {
    createQRcode (item, node) {
      if (item.children != null && item.children.length > 0) {
        return
      }
      const tableParent = this.getCurrentParentByTag('table[data-wk-table-tag="table"]')
      if (!tableParent) {
        this.editor.insertContent(this.getQRcode(item, node))
    ...
    getQRcode (item, node) {
      ...
      // return '<div ref=\"qrcode\" class=\"qrcode\"></div>'
      return `<span ref="${item.field}" id="${item.field}" data-tag="${parent}.${item.field}" class="wk-print-tag-wukong ${this.getSpanColorClass()}" contenteditable="false">{${item.field}}</span>`
    },
