<template>
  <el-dialog :title="!dataForm.id ? '新建单据' : '编辑单据'"
    ...
    <el-form ref="dataForm"
      ...
      <el-form-item label="流水前辍"
                    prop="prefix">
        <el-input v-model="dataForm.prefix"
                  placeholder="输入前缀"
                  @keyup.native="handleChange" />
      </el-form-item>
      <el-form-item label="流水日期"
                    prop="dateFormat">
        <el-select v-model="dataForm.dateFormat"
                   placeholder="请选择"
                   clearable
                   @change="handleChange">
          <el-option label="yyyymmdd"
                     value="yyyyMMdd" />
          <el-option label="yyyymm"
                     value="yyyyMM" />
      ...
      <el-form-item label="流水位数"
                    prop="digit">
        <el-input v-model.number="dataForm.digit"
                  maxlength="1"
                  placeholder="流水位数"
                  @keyup.native="handleChange" />
      </el-form-item>
      <el-form-item label="流水起始"
                    prop="startNumber">
        <el-input v-model="dataForm.startNumber"
                  placeholder="不允许输入0或特殊字符"
                  @keyup.native="handleChange" />
      </el-form-item>
      <el-form-item label="流水范例"
                    prop="example">
        <el-input v-model="dataForm.example"
                  disabled />
      </el-form-item>
