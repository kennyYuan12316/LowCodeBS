...
export default {
  ...
  methods: {
    datesRender(calendar) {
      ...
      this.startTime = this.hsz.toDate(view.activeStart, "yyyy-MM-dd HH:mm")
      this.endTime = this.hsz.toDate(view.activeEnd, "yyyy-MM-dd HH:mm")
      this.initData()
    },
    handleDateClick(arg) {
      let startTime = this.hsz.toDate(arg.date, "yyyy-MM-dd HH:00"),
        clickTime = this.hsz.toDate(arg.date, "yyyyMMdd"),
        currTime = this.hsz.toDate(new Date(), "yyyyMMdd");
      if (clickTime < currTime) return
      if (clickTime == currTime) {
        let thisDate = new Date();
        thisDate.setHours(thisDate.getHours() + 1);
        startTime = this.hsz.toDate(thisDate, "yyyy-MM-dd HH:00");
      }
      this.addOrUpdateHandle('', startTime)
      this.addOrUpdateHandle('', new Date(startTime).getTime())
    },
    eventClick(data) {
      if (this.hsz.toDate(data.event.start, "yyyyMMddHHmm") > this.hsz.toDate(new Date(), "yyyyMMddHHmm")) {
