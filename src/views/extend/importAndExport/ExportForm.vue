<template>
  <el-dialog title="导出数据"
             ...
             class="HSZ-dialog HSZ-dialog_center HSZ-dialog-export"
             lock-scroll
             width="600px">
    <el-form label-position="top">
      ...
      <el-form-item class="export-line">
        <p slot="label"
           class="export-label">列表数据<span>请选择导出字段</span></p>
        <el-checkbox :indeterminate="isIndeterminate"
                     v-model="checkAll"
                     @change="handleCheckAllChange">全选</el-checkbox>
        <el-checkbox-group v-model="columns"
                           @change="handleCheckedChange">
          <el-checkbox v-for="item in columnList"
                       :label="item.prop"
                       :key="item.prop"
                       class="column-item">
            {{item.label}}
    ...
    <span slot="footer"
      ...
      <el-button type="primary"
                 @click="downLoad"
                 :loading="btnLoading">导 出</el-button>
    </span>
  </el-dialog>
</template>
