  ...
  <div class="app-container Email-container nohead">
    <el-tabs tab-position="left"
      ...
      <el-tab-pane name="writeMail">
        <span slot="label"><i class="icon-sz icon-sz-btn-edit"></i>写邮件</span>
      </el-tab-pane>
      <el-tab-pane name="inBox">
        <span slot="label"><i class="icon-sz icon-sz-extend-envelope"></i>收件箱</span>
      </el-tab-pane>
      <el-tab-pane name="star">
        <span slot="label"><i class="icon-sz icon-sz-extend-star"></i>星标件</span>
      </el-tab-pane>
      <el-tab-pane name="draft">
        <span slot="label"><i class="icon-sz icon-sz-extend-exclamation-triangle"></i>草稿箱</span>
      </el-tab-pane>
      <el-tab-pane name="sent">
        <span slot="label"><i class="icon-sz icon-sz-extend-paper-plane"></i>已发送</span>
      </el-tab-pane>
      <el-tab-pane name="setAccount">
        <span slot="label"><i class="icon-sz icon-sz-extend-cog"></i>邮箱配置</span>
      </el-tab-pane>
      <div class="main HSZ-flex-main">
          ...
          <el-form @submit.native.prevent>
            ...
            <el-col :span="6">
              <el-form-item label="日期">
                <el-date-picker v-model="pickerVal"
                                type="daterange"
        ...
        <HSZ-table v-loading="listLoading"
            ...
            <template slot-scope="scope">
              <template v-if="activeTab==='inBox'|| activeTab==='star'">
                <span v-if=" scope.row.isRead">
                  <i class="icon-sz icon-sz-extend-envelope-open-o i-default"
                     title='点击标记为未读'
                     @click="ReceiveUnread(scope.row)"></i></span>
                <span v-else><i class="icon-sz icon-sz-extend-envelope text-warning"
                     title='点击标记为已读'
                     @click="ReceiveRead(scope.row)"></i></span>
              </template>
              <span v-else><i class="icon-sz icon-sz-extend-envelope-open-o i-default"></i></span>
            </template>
          </el-table-column>
          <el-table-column label=""
                           width="40"
                           prop="attachment">
            <template slot-scope="scope">
              <span v-if="scope.row.attachment && JSON.parse(scope.row.attachment).length"><i class="icon-sz icon-sz-extend-paperclip i-default"></i></span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="subject"
            ...
            <template slot-scope="scope">
              <el-link @click="write(scope.row.id)"
          ...
          <el-table-column label=""
                           ...
                           v-if="activeTab==='inBox'|| activeTab==='star'">
            <template slot-scope="scope">
              <span v-if="scope.row.starred"><i class="icon-sz icon-sz-extend-star text-warning img-star"
                   title='点击取消星标'
                   @click="ReceiveNoStarred(scope.row,scope.$index)"></i></span>
              <span v-else><i class="icon-sz icon-sz-extend-star-o i-default img-star"
                   title='点击标记为星标邮件'
                   @click="ReceiveYesStarred(scope.row)"></i></span>
            </template>
          </el-table-column>
