      ...
      <div class="HSZ-common-layout-main  HSZ-flex-main">
        ...
        <HSZ-table v-loading="listLoading"
          ...
          <el-table-column label="项目标记"
            ...
            <template slot-scope="scope">
              <template v-if="scope.row.signArray.length">
                <span v-for="(item,i) in scope.row.signArray"
                      :key="i">
                  <i class="sz-custom sz-custom-checkbox-blank-circle"
                     style="color:#ff625c;font-size:16px;"
                     v-if="item=='1'"></i>
                  <i class="sz-custom sz-custom-checkbox-blank-circle"
                     style="color:#f9a646;font-size:16px;"
                     v-if="item=='2'"></i>
                  <i class="sz-custom sz-custom-checkbox-blank-circle"
                     style="color:#f4ce4a;font-size:16px;"
                     v-if="item=='3'"></i>
                  <i class="sz-custom sz-custom-checkbox-blank-circle"
                     style="color:#6dcc51;font-size:16px;"
                     v-if="item=='4'"></i>
                  <i class="sz-custom sz-custom-checkbox-blank-circle"
                     style="color:#4bb8f3;font-size:16px;"
                     v-if="item=='5'"></i>
                  <i class="sz-custom sz-custom-checkbox-blank-circle"
                     style="color:#d088e1;font-size:16px;"
                     v-if="item=='6'"></i>
                  <i class="sz-custom sz-custom-checkbox-blank-circle"
                     style="color:#a5a5a8;font-size:16px;"
                     v-if="item=='7'"></i>
                </span>
              </template>
              <i class="sz-custom sz-custom-checkbox-blank-circle-outline"
                 v-else
                 style="font-size:16px;color:#cecece"></i>
            </template>
          </el-table-column>
          <el-table-column prop="projectPhase"
                           label="项目阶段"
                           sortable
                           width="120" />
          ...
          <el-table-column prop="principal"
                           ...
                           sortable
                           width="80" />
          <el-table-column prop="jackStands"
                           label="立项人"
                           sortable
                           width="80" />
          <el-table-column label="交互时间"
                           sortable
                           width="100">
            <template slot-scope="scope">
          ...
          <el-table-column prop="costAmount"
                           label="费用金额"
                           sortable
                           width="100" />
          <el-table-column prop="tunesAmount"
                           label="已用金额"
                           sortable
                           width="100" />
          <el-table-column prop="projectedIncome"
                           label="预计收入"
                           sortable
                           width="100" />
          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              <el-dropdown>
                <el-button size="mini"
                           type="text">项目标记<i class="el-icon-arrow-down el-icon--right"></i>
                </el-button>
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-for="(item,i) in options"
                                    :key="item.value"
...
import HSZForm from '../commonForm'
export default {
  ...
  methods: {
    ...
    addOrUpdateHandle (id) {
      ...
      this.$nextTick(() => {
        this.$refs.HSZForm.init(this.industryTypeList, id)
      })
    },
