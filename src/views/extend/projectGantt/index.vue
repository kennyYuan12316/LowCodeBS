      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        ...
        <HSZ-table v-loading="listLoading"
          ...
          <el-table-column label="状态"
            ...
            <template slot-scope="scope">
              <el-tag type="success"
                      v-if="scope.row.schedule==100">已完成</el-tag>
              <template v-else>
                <el-tag v-if="scope.row.state==1">进行中</el-tag>
                <el-tag type="danger"
                        v-if="scope.row.state==2">已暂停</el-tag>
              </template>
            </template>
          </el-table-column>
          <el-table-column prop="managerIds"
                           label="参与人员">
            <template slot-scope="scope">
              <el-avatar :size="30"
                         :src="define.comUrl+img.headIcon"
                         v-for="(img,i) in scope.row.managersInfo"
                         :key="i"
                         :title="img.account"
