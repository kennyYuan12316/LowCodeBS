<template>
  <div class="HSZ-common-layout">
      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        <div class="HSZ-common-head">
          <topOpts @add="addOrUpdateHandle()">
            <upload-btn url="/api/system/DataInterface/Action/Import"
                        @on-success="getList" />
          </topOpts>
              ...
              <tableOpts @edit="addOrUpdateHandle(scope.row.id)"
                ...
                <el-dropdown hide-on-click>
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item @click.native="handlePreview(scope.row.id, scope.row.enabledMark,scope.row.tenantId||'')">
                      预览
                    </el-dropdown-item>
    ...
    <Form v-if="formVisible"
          ref="Form"
          @close="closeForm" />
    <Log v-show="logVisible"
         ref="Log"
         @close="logVisible=false" />
    <Preview v-if="previewVisible"
             ref="Preview"
             @close="closePreview" />
  </div>
</template>
