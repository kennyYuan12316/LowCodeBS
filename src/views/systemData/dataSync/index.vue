<template>
  <div class="DbCopy-container app-container nohead  HSZ-flex-main">
    <el-alert title="温馨提示：数据同步是由A数据库同步到B数据库，必须确保数据库表结构一致否则会同步失败。"
              type="warning"
              :closable="false"
              show-icon
              class="mb-20">
    ...
    <el-form ref="dataForm"
             ...
             class="mt-10">
      <el-form-item label="数据库连接 From"
                    prop="dbConnectionFrom">
        <el-select v-model="dataForm.dbConnectionFrom"
                   placeholder="请选择连接">
      ...
      <el-form-item label="数据库连接 To"
                    prop="dbConnectionTo">
        <el-select v-model="dataForm.dbConnectionTo"
                   placeholder="请选择连接">
          <el-option-group v-for="group in dbOptions"
                           :key="group.fullName"
                           :label="group.fullName">
        ...
        <el-button type="primary"
                   @click="check"
                   style="margin-left:10px">验证连接</el-button>
      </el-form-item>
    </el-form>
