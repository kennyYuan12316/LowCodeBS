  ...
  <el-drawer title="字典分类管理"
    ...
    <div class="HSZ-flex-main">
      <div class="HSZ-common-head">
        <topOpts @refresh="getDictionaryTypeList()"
                 @add="addOrUpdateHandle()">
      ...
      <HSZ-table v-loading="listLoading"
        ...
        <el-table-column prop="isTree"
          ...
          <template slot-scope="scope">
            <el-tag v-if="scope.row.isTree === 1"
                    type="success"
                    size="mini">是</el-tag>
            <el-tag v-else
                    type="danger"
        ...
        <el-table-column label="操作"
          ...
          <template slot-scope="scope">
            <tableOpts @edit="addOrUpdateHandle(scope.row.id)"
              ...
              <el-button size="mini"
                         type="text"
                         @click="exportData(scope.row.id)">导出
              </el-button>
            </tableOpts>
          </template>
        </el-table-column>
      </HSZ-table>
      <TypeForm v-if="typeFormVisible"
                ref="TypeForm"
                @refreshDataList="getDictionaryTypeList" />
    </div>
  </el-drawer>
</template>
<script>
