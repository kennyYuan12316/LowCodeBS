      ...
      <div class="HSZ-common-layout-main HSZ-flex-main">
        <div class="HSZ-common-head">
          <div v-if="isPreview || !columnData.useBtnPermission">
            <el-button :type="i==0?'primary':'text'"
                       :icon="item.icon"
        ...
        <HSZ-table v-loading="listLoading"
          ...
          <el-table-column prop="flowState"
            ...
            <template slot-scope="scope"
              ...
              <el-tag type="success"
                      v-else-if="scope.row.flowState==2">审核通过</el-tag>
              <el-tag type="danger"
                      v-else-if="scope.row.flowState==3">审核驳回</el-tag>
              <el-tag type="danger"
                      v-else-if="scope.row.flowState==4">流程撤回</el-tag>
              <el-tag type="warning"
                      v-else-if="scope.row.flowState==5">审核终止</el-tag>
              <el-tag type="info"
                      v-else>等待提交</el-tag>
            </template>
          </el-table-column>
                ...
                <template v-if="customBtnsList.length">
                  <el-dropdown hide-on-click>
                    ...
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item v-for="(item, i) in customBtnsList"
                                        :key="i"
...
export default {
  ...
  methods: {
    ...
    customBtnsHandel (item, row, index) {
      ...
      const func = this.hsz.getScriptFunc.call(this, item.func)
      if (!func) return
