<template>
  <div>
    <template v-if="formData.popupType==='general'">
      <el-dialog title="详情" :close-on-click-modal="false" :visible.sync="visible" class="HSZ-dialog HSZ-dialog_center"
        lock-scroll :width="formData.generalWidth" append-to-body>
    ...
    <template v-if="formData.popupType==='fullScreen'">
      <transition name="el-zoom-in-center">
        <div class="HSZ-preview-main">
          ...
          <div class="dynamic-form-main dynamicDetail" :style="{margin: '0 auto',width:formData.fullScreenWidth}"
    ...
    <template v-if="formData.popupType==='drawer'">
      <el-drawer title="详情" :visible.sync="visible" :wrapperClosable="false" :size='formData.drawerWidth' append-to-body
  ...
  methods: {
    ...
    handleAttrList(list) {
      ...
      for (let i = 0; i < realList.length; i++) {
        ...
        let modelId = '', id = "", field = "", hszKey = '', activeItem = {}
        let prop = item.relationField.split('_hszTable_')[0]
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            if (prop === list[i].__vModel__) {
              hszKey = list[i].__config__.hszKey
              modelId = list[i].__config__.hszKey === 'relationForm' ? list[i].modelId : list[i].interfaceId
              id = list[i].__config__.defaultValue
              field = list[i].__config__.tableName ? list[i].__vModel__ + '_hszTable_' + list[i].__config__.tableName + (list[i].__config__.isSubTable ? '0' : "1") : list[i].__vModel__
              activeItem = list[i]
              break
            }
            if (list[i].__config__ && list[i].__config__.hszKey !== 'table' && list[i].__config__.children && Array.isArray(list[i].__config__.children)) {
    ...
    fillFormData(form, data) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          let item = list[i]
          if (item.__vModel__) {
            if (item.__config__.hszKey === 'relationForm' || item.__config__.hszKey === 'popupSelect') {
          ...
          if (['relationFormAttr', 'popupAttr'].includes(item.__config__.hszKey)) relationFormAttrList.push(item)
