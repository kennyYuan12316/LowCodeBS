  ...
  <div class="columnDesign-container">
    <div class="main-board">
      <div class="search-box">
        ...
        <el-table :data="columnData.searchList"
          ...
          <el-table-column align="center"
                           ...
                           width="50">
            <template>
              <i class="drag-handler icon-sz icon-sz-darg"
                 style="cursor: move;font-size:20px"
                 title='点击拖动' />
            </template>
          </el-table-column>
          <el-table-column prop="__config__.label"
                           label="列名" />
          <el-table-column prop="__vModel__"
                           label="字段" />
      ...
      <div class="column-box">
        ...
        <el-table :data="columnData.columnList"
                  class="HSZ-common-table"
                  ref="dragTable"
    ...
    <div class="right-board">
      <el-tabs v-model="currentTab"
        ...
        <el-tab-pane label="列表字段"
                     name="field" />
        <el-tab-pane label="列表属性"
                     name="column" />
      </el-tabs>
      <div class="field-box">
        <div class="searchList"
             v-show="currentTab==='search'">
          <el-table :data="searchOptions"
                    class="HSZ-common-table"
                    height="100%"
        ...
        <el-scrollbar class="right-scrollbar"
                      v-show="currentTab==='column'">
          <div class="setting-box">
            <el-form :model="columnData"
                     label-width="80px">
              ...
              <template v-if="columnData.type==2">
                ...
                <el-form-item label="左侧标题">
                  <el-input v-model="columnData.treeTitle"
                            placeholder="树形标题"></el-input>
                </el-form-item>
                <el-form-item label="数据来源">
                  <el-select v-model="columnData.treeDataSource"
                             placeholder="请选择数据来源"
                             @change="dataTypeChange">
                    <el-option label="数据字典"
                               value="dictionary"></el-option>
                    <el-option label="远端数据"
                               value="api"></el-option>
                    <el-option label="组织数据"
                               value="organize"></el-option>
                    <el-option label="部门数据"
                               value="department"></el-option>
                  </el-select>
                </el-form-item>
                <template v-if="columnData.treeDataSource==='dictionary'">
                  <el-form-item label="数据字典">
                    <DicSelect v-model="columnData.treeDictionary"
                               placeholder="请选择数据字典" />
                  </el-form-item>
                  <el-form-item label="主键字段">
                    <el-select v-model="columnData.treePropsValue"
                               placeholder="请选择主键字段">
                      <el-option label="id"
                                 value="id"></el-option>
                      <el-option label="enCode"
                                 value="enCode"></el-option>
                    </el-select>
                  </el-form-item>
                </template>
                <template v-if="columnData.treeDataSource==='api'">
                  <el-form-item label="数据接口">
                    <HSZ-TreeSelect :options="dataInterfaceSelector"
                                    v-model="columnData.treePropsUrl"
                                    placeholder="请选择数据接口"
                                    lastLevel
                  ...
                  <el-form-item label="主键字段">
                    <el-input v-model="columnData.treePropsValue"
                              placeholder="主键字段" />
                  </el-form-item>
                  <el-form-item label="显示字段">
                    <el-input v-model="columnData.treePropsLabel"
                              placeholder="显示字段" />
                  </el-form-item>
                  <el-form-item label="子级字段">
                    <el-input v-model="columnData.treePropsChildren"
                              placeholder="子级字段" />
                  </el-form-item>
                </template>
                <el-form-item label="关联字段">
                  <el-select v-model="columnData.treeRelation"
                             placeholder="请选择关联字段">
              ...
              <template v-if="columnData.type==3">
                ...
                <el-form-item label="分组字段">
                  <el-select v-model="columnData.groupField"
                             placeholder="请选择分组字段">
                    <el-option :label="item.__config__.label"
                               :value="item.__vModel__"
              ...
              <el-form-item label="排序字段">
                <el-select v-model="columnData.defaultSidx"
                           placeholder="请选择排序字段"
                           clearable>
                  <el-option :label="item.__config__.label"
                             :value="item.__vModel__"
              ...
              <el-form-item label="排序类型">
                <el-select v-model="columnData.sort"
                           placeholder="请选择排序类型">
                  <el-option label="升序"
                             value="asc"></el-option>
                  <el-option label="降序"
              ...
              <el-checkbox-group v-model="btnsList"
                                 class="btnsList">
              ...
              <el-checkbox-group v-model="columnBtnsList"
                                 class="btnsList columnBtnList">
                <el-checkbox :label="item.value"
              ...
              <template v-if="modelType==1">
                ...
                <div class="custom-btns-list">
                  <draggable :list="columnData.customBtnsList"
                             :animation="340"
                             group="customItem"
                             handle=".option-drag">
                    <div v-for="(item, index) in columnData.customBtnsList"
                         :key="index"
                         class="custom-item">
                      <div class="custom-line-icon option-drag">
                        <i class="icon-sz icon-sz-darg" />
                      </div>
                      ...
                      <el-button class="custom-btn"
                                 @click="editFunc(item)">脚本事件</el-button>
                ...
                <div>
                  <el-button type="text"
                             icon="el-icon-plus"
                             @click="addCustomBtn">添加按钮</el-button>
                </div>
              </template>
...
const getSearchType = item => {
  const hszKey = item.__config__.hszKey
  // 等于-1  模糊-2  范围-3
  const fuzzyList = [...useInputList]
  const RangeList = [...useDateList, 'time', 'date', 'numInput', 'calculate']
  if (RangeList.includes(hszKey)) return 3
  if (fuzzyList.includes(hszKey)) return 2
  return 1
}
...
export default {
  ...
  created () {
    ...
    const loop = (data, parent) => {
      ...
      if (data.__config__ && data.__config__.hszKey) {
    ...
    loop(getDrawingList())
    this.list = list
    let columnOptions = list.filter(o => noColumnShowList.indexOf(o.__config__.hszKey) < 0)
    let searchOptions = list.filter(o => noSearchList.indexOf(o.__config__.hszKey) < 0)
    this.columnOptions = columnOptions.map(o => ({
      label: o.__config__.label,
      prop: o.__vModel__,
      align: 'left',
      hszKey: o.__config__.hszKey,
      sortable: false,
      width: null
    }));
