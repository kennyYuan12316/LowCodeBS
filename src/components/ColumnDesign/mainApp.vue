  ...
  <div class="columnDesign-container">
    <div class="main-board">
      <div class="sort-box">
        ...
        <el-table :data="columnData.sortList"
    ...
    <div class="right-board">
      ...
      <div class="field-box">
        <div class="columnList"
             v-show="currentTab==='sort'">
          <el-table :data="sortOptions"
                    class="HSZ-common-table"
                    height="100%"
                    @selection-change="sortSelectionChange"
                    ref="sortTable">
            <el-table-column prop="label"
                             label="排序字段" />
  ...
  created () {
    ...
    const loop = (data, parent) => {
      ...
      if (data.__config__ && data.__config__.hszKey) {
        ...
        if (data.__config__.layout === "colFormItem" && data.__vModel__ && data.__vModel__.indexOf('_hsz_') < 0) {
          list1.push(data)
        }
      }
    }
    loop(getDrawingList())
    this.list = list
    let options = list.filter(o => noColumnShowList.indexOf(o.__config__.hszKey) < 0)
    let searchOptions = list.filter(o => noSearchList.indexOf(o.__config__.hszKey) < 0)
    let sortOptions = list1.filter(o => noColumnShowList.indexOf(o.__config__.hszKey) < 0)
