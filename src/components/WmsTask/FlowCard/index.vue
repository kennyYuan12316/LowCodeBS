...
function createNormalCard (ctx, conf, h) {
  ...
  const isDynamicNode = afterTrue(NodeUtils.isDynamicNode(conf), 'dynamic')
  return (
          ...
          <div class="condition-box">
            ...
            <div>
              <div class="condition-icon" onClick={ctx.eventLauncher.bind(ctx, "addApprovalNode", data, isBranch)} >
                <i class="icon-sz icon-sz-mapExample"></i>
              </div>
              子任务路径
            </div>
            <div class={{ 'condition-disabled': !canAddAppendBranch }} >
              <div class="condition-icon" onClick={this.eventLauncher.bind(ctx, "appendBranch", data, isBranch, !canAddAppendBranch)}>
                <i class="sz-custom sz-custom-sitemap"></i>
              </div>
              条件分支
            </div>

            <div>
              <div class="condition-icon" onClick={ctx.eventLauncher.bind(ctx, "addDynamicNode", data, isBranch)} >
                <i class="icon-sz icon-sz-mapExample"></i>
              </div>
              动态路径
            </div>
...
function NodeFactory (ctx, data, h) {
  ...
  let res = [],
    branchNode = "",
    content = NodeUtils.isConditionNode(data) ? "未设置条件" : 
    NodeUtils.isSubFlowNode(data) ? "未设置发起人" : 
    NodeUtils.isDynamicNode(data) ? "未设置业务类型" : 
    "未设置审批人",
    selfNode = (
      <div class="node-wrap">
...
function addEndNode (h) {
  return <section class="end-node">任务结束</section>;
}
