...
<template>
  <el-drawer size="550px"
    ...
    <header slot="title"
            class="header"
            v-if="value && (value.type=='condition'|| value.type=='approver' || value.type=='subFlow' || value.type=='dynamic' || value.type=='start' )">
      <el-input size="mini"
      ...
      <el-tabs style="height:100%;">
        <el-tab-pane label="基础设置"
          ...
          <el-form-item label="路径">
            <popupSelect
              v-model="initiatePath"
              field="initiatePath"
              placeholder="请选择路径"
              clearable
              required
              relationField="StationFrom"
              :columnOptions="workpathOptions"
              :pageSize="20"
              propsValue="PathID"
              interfaceId="344076277537309957"
              popupType="dialog"
              popupTitle="选择路径"
              popupWidth="800px"
            >
    ...
    <section class="approver-pane"
      ...
      <el-tabs v-model="activeName"
        ...
        <el-tab-pane label="基础设置"
          ...
          <el-form-item label="路径">
            <PopupSelect
              v-model="approverForm.approverPath"
              field="approverPath"
              placeholder="请选择路径"
              clearable
              required
        ...
        <el-col :span="24">
          <el-form-item label="任务类型">
							<el-select v-model='approverForm.taskType' placeholder='请选择' @change="$forceUpdate()" clearable required :style='{"width":"100%"}' >
      ...
      <el-tabs v-model="activeName"
        ...
        <el-col :span="24">
          <el-form-item label="任务类型">
							<el-select v-model='dynamicForm.taskType' placeholder='请选择' @change="$forceUpdate()" clearable required :style='{"width":"100%"}' >
								<el-option v-for="(item, index) in workTypeOptions" :key="index" :label="item.fullName" :value="item.enCode" ></el-option>
							</el-select>
						</el-form-item>
        </el-col>
        <!-- <el-col :span="24">
          <el-form-item label="绑定接驳台(rgv)">
            <PopupSelect
              v-model="dynamicForm.bindEndSite"
              field="bindEndSite"
              placeholder="请选择站点"
              clearable
              required
        ...
        <el-col :span="24">
						<el-form-item label="绑定接驳台(rgv)" prop="bindEndSite">
							<el-select v-model='dynamicForm.bindEndSite' placeholder='请选择' @change="$forceUpdate()" multiple filterable  clearable :style='{"width":"100%"}' >
								<el-option v-for="(item, index) in siteOptions" :key="index" :label="item.Capion" :value="item.StationID" ></el-option>
							</el-select>
						</el-form-item>
					</el-col>
      </el-form>
...
export default {
  ...
  components: { OrgSelect, MsgDialog, InterfaceDialog,PopupSelect },
  data () {
    return {
      ...
      initiatePath: [],
      workpathOptions: [
        { value: "PathID", label: "路径编号" },
        { value: "StationFrom", label: "路径名称" },
        { value: "StationTo", label: "起点设备" },
        { value: "DeviceFrom", label: "起点站点" },
        { value: "DeviceTo", label: "终点站点" },
      ...
      dynamicForm: JSON.parse(JSON.stringify(defaultDynamicForm)),
  ...
  computed: {
    ...
    usedFormItems () {
      ...
      let list = [{
        __config__:{label:"返回判断值"},
        __vModel__:"ResultValue"
      }]
  ...
  methods: {
    ...
    resetOrgColl () {
      ...
      this.approverForm.approverPath = undefined
      this.approverForm.taskType = undefined

      this.dynamicForm.taskType = undefined
      this.dynamicForm.bindEndSite = undefined
    },
    ...
    startNodeConfirm () {
      ...
      this.properties.initiatePath = this.initiatePath
      let content = '',
     ...
     */
     dynamicodeConfirm  () {
      ...
      Object.assign(this.properties, this.dynamicForm, titleObj)
      this.$emit("confirm", this.properties, content|| '请设置动态节点');
      this.visible = false;
    },
    /**
     * 子流程节点确认保存
    ...
    approverNodeConfirm () {
      ...
      if (assigneeType == 6) {
        ...
        if (!this.approverForm.approverPath) {
          this.$message({
            message: '请设置子任务路径',
            type: 'error',
          })
        ...
        var checkRow=this.$refs['popupSelect']['selectedLabels']
    ...
    confirm () {
      ...
      this.isDynamicNode() && this.dynamicodeConfirm()
    ...
    isDynamicNode () {
      return this.value ? NodeUtils.isDynamicNode(this.value) : false
    },
    // 初始化发起人节点
    initInitiator () {
      ...
      this.initiatePath = this.value.properties && this.value.properties.initiatePath
    ...
    */
    ...
    initDynamicData(){
      this.getworkTypeOptions();
      this.getsiteOptions();
      Object.assign(this.dynamicForm, this.value.properties)
  ...
  watch: {
    visible (val) {
      if (!val) {
        ...
        this.dynamicForm = JSON.parse(JSON.stringify(defaultDynamicForm))

        return
      }
      this.isStartNode() && this.initStartNodeData()
      this.isSubFlowNode() && this.initSubFlowData()
      this.isDynamicNode() && this.initDynamicData()
