...
const layouts = {
  colFormItem (h, scheme) {
    ...
    const Item = config.hszKey === 'cascader'
  ...
  rowFormItem (h, scheme) {
    ...
    if (scheme.__config__.hszKey === 'tab') {
      return (
    ...
    if (scheme.__config__.hszKey === 'collapse') {
      return (
    ...
    if (scheme.__config__.hszKey === 'table') {
    ...
    if (scheme.__config__.hszKey === 'card') {
      return (
    ...
    if (scheme.__config__.hszKey === 'row') {
      if (scheme.type === 'flex') {
...
function buildListeners (scheme) {
  ...
  if (scheme.on) {
    ...
    Object.keys(scheme.on).forEach(key => {
      ...
      const func = this.hsz.getScriptFunc.call(this, str);
      if (!func) return
      listeners[key] = params => {
        if (key === 'change') {
          let data = ''
          if (['select', 'radio', 'checkbox'].includes(config.hszKey)) {
            const options = scheme.__slot__.options
            if (scheme.multiple || config.hszKey === 'checkbox') {
              let _data = []
          ...
          } else if (config.hszKey === 'numInput') {
            data = params[0]
          } else {
  ...
  methods: {
    initFormData (componentList, formData) {
      ...
      componentList.forEach(cur => {
        ...
        if (cur.__config__.hszKey == 'table') return
    ...
    buildRules (componentList, rules) {
      componentList.forEach(cur => {
        ...
        if (config.children && config.hszKey !== 'table') this.buildRules(config.children, rules)
      })
    },
    onLoad (formConfCopy) {
      if (!formConfCopy || !formConfCopy.funcs || !formConfCopy.funcs.onLoad) return
      const onLoadFunc = this.hsz.getScriptFunc.call(this, formConfCopy.funcs.onLoad)
    ...
    comSet (field, prop, value) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (item.__vModel__ && item.__vModel__ === prop) {
            switch (field) {
              case 'disabled':
                ...
                break;
              case 'options':
                if (dyOptionsList.indexOf(item.__config__.hszKey) > -1) {
          ...
          if (item.__config__ && item.__config__.hszKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
    ...
    beforeSubmit () {
      ...
      const func = this.hsz.getScriptFunc.call(this, this.formConfCopy.funcs.beforeSubmit)
    ...
    afterSubmit () {
      ...
      const func = this.hsz.getScriptFunc.call(this, this.formConfCopy.funcs.afterSubmit)
      if (!func) return
      func(this.parameter)
    },
    submitForm () {
    ...
    onCascaderChange (data, on) {
      ...
      const func = this.hsz.getScriptFunc.call(this, on.change)
      if (!func) return
      func.call(this, { data, ...this.parameter })
    },
    onCascaderBlur (data, on) {
      if (!on || !on.blur) return
      const func = this.hsz.getScriptFunc.call(this, on.blur)
      if (!func) return
      func.call(this, { data, ...this.parameter })
    }
  },
