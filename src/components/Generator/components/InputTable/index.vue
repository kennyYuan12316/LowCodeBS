  ...
  <div class="hsz-table-box"
    ...
    <el-table :data="tableFormData"
      ...
      <el-table-column width="50"
        ...
        <template slot-scope="scope">
          <div class="row-action">
            <span class="index"
                  :class="{'btn-disabled':disabled}"> {{ scope.$index + 1 }}</span>
      ...
      <template v-for="(head, cindex) in tableData">
        <el-table-column :key="head.__config__.formId"
          ...
          <template slot="header">
            <span style="color: #f56c6c;"
                  v-if="head.__config__.required">*</span>
            {{ head.__config__['label'] }}
          </template>
          <template slot-scope="scope">
            <!-- 单选框组 多选框组 都替换成下拉 并添加options -->
            <template v-if="['select', 'checkbox','radio'].includes(head.__config__.hszKey)">
              <el-select v-model="tableFormData[scope.$index][cindex].value"
                ...
                <el-option v-for="(opt,oindex) in head.__slot__.options"
                           :key="oindex"
            ...
            <!-- 单行输入 -->
            <template v-else-if="head.__config__.hszKey==='comInput'">
              <el-input v-model="tableFormData[scope.$index][cindex].value"
                ...
                <template v-if="head.__slot__">
                  <template slot="prepend"
                            v-if="head.__slot__.prepend">
                    {{ head.__slot__.prepend }}
                  </template>
                  <template slot="append"
                            v-if="head.__slot__.append">
    ...
    <div class="table-actions"
         ...
         v-if="!disabled">
      <el-button type="text"
                 icon="el-icon-plus"> {{ config.actionText }}</el-button>
    </div>
  </div>
</template>
<script>
  ...
  methods: {
        ...
        if (dyOptionsList.indexOf(config.hszKey) > -1) {
          ...
          if (config.dataType === 'dynamic') {
            ...
            getDataInterfaceRes(config.propsUrl).then(res => {
              let realData = this.hsz.interfaceDataHandler(res.data)
    ...
    setTableFormData (prop, value) {
      ...
      for (let i = 0; i < activeRow.length; i++) {
        ...
        if (activeRow[i].__vModel__.indexOf('_hszRelation_') >= 0) {
          vModel = activeRow[i].__vModel__.substring(0, activeRow[i].__vModel__.indexOf('_hszRelation_'))
        }
        if (vModel === prop) {
    ...
    getTableFieldOptions (prop) {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        if (this.tableData[i].__vModel__ === prop) {
          ...
          let isTreeSelect = item.__config__.hszKey === 'treeSelect' || item.__config__.hszKey === 'cascader'
    ...
    onFormBlur (rowIndex, colIndex, tag) {
      ...
      if (data && data.on && data.on.blur) {
        const func = this.hsz.getScriptFunc.call(this, data.on.blur)
        if (!func) return
        func.call(this, {
          data: null,
          ...this.parameter
    ...
    onFormDataChange (rowIndex, colIndex, tag, params) {
      ...
      if (data && data.on && data.on.change) {
        const func = this.hsz.getScriptFunc.call(this, data.on.change)
        if (!func) return
        let value = ''
        if (['select', 'radio', 'checkbox'].includes(data.hszKey)) {
          const options = data.options
          if (data.config.multiple || data.hszKey === 'checkbox') {
            let _value = []
    ...
    getConfById (formId, rowIndex) {
      ...
      if (['relationForm', 'popupSelect'].includes(itemConfig.hszKey)) {
        newObj['field'] = item.__vModel__ + '_hszRelation_' + rowIndex
      }
      if (['relationFormAttr', 'popupAttr'].includes(itemConfig.hszKey)) {
        let prop = newObj['relationField'].split('_hszTable_')[0]
        newObj['relationField'] = prop + '_hszRelation_' + rowIndex
      }
      return newObj
    },
    /**
    ...
    getEmptyRow (val, rowIndex) {
      return this.tableData.map((t, index) => {
        let res = {
          ...
          regErrorText: '',
          on: t.on || {},
          hszKey: t.__config__.hszKey,
          __vModel__: ['relationForm', 'popupSelect'].includes(t.__config__.hszKey) ? t.__vModel__ + '_hszRelation_' + rowIndex : t.__vModel__,
    ...
    getTableValue () {
      return this.tableFormData.map(row => row.reduce((p, c) => {
        ...
        if (c.__vModel__ && c.__vModel__.indexOf('_hszRelation_') >= 0) {
          str = c.__vModel__.substring(0, c.__vModel__.indexOf('_hszRelation_'))
        }
        p[str] = c.value
        return p
      }, {}))
