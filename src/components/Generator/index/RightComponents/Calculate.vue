    ...
    <el-form-item label="计算公式">
      <div @click="expDialogVisible = true"
        ...
        <template v-if="activeData.expression.length">
          <span v-for="(item, index) in activeData.expression"
                :key="index"
        ...
        <span v-else
              class="placeholder-text">编辑计算公式</span>
      </div>
    </el-form-item>
    <el-form-item label="是否隐藏">
      <el-switch v-model="activeData.__config__.noShow" />
    </el-form-item>
    <el-dialog title="编辑计算公式"
      ...
      <div class="calc-box">
        <div class="calc-preview"
             :class="{error: !expValid}">
          计算公式 =
          <span v-for="(item, index) in expressionTemp"
                :key="index"
                :class="{'calc-btn': typeof item !== 'string',
            'error': typeof item !== 'string' && item.label === '无效的值'}">
            {{typeof item !== 'string' ? item.label : item}}
        ...
        <div>
          <span>计算对象：</span>
          <span v-for="item in calculateCmps"
                :key="item.__vModel__"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="checkExpression">确 定</el-button>
      </span>
    </el-dialog>
  </el-row>
</template>
    ...
    calculateCmps () {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__config__ && data.__config__.hszKey) {
          if (['numInput', 'calculate'].includes(data.__config__.hszKey)) {
            const isTableChild = parent && parent.__config__ && parent.__config__.hszKey === 'table'
            calcList.push({
