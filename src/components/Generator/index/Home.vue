  ...
  <div class="container">
    ...
    <div class="center-board">
      <div class="action-bar">
        <el-button icon="icon-sz icon-sz-pc"
                   :class="{'unActive-btn':showType!='pc'}"
                   type="text"
                   @click="showType='pc'"
                   size="medium"></el-button>
        <el-button icon="icon-sz icon-sz-mobile"
                   :class="{'unActive-btn':showType!='app'}"
                   type="text"
                   @click="showType='app'"
                   size="medium"></el-button>
        ...
        <el-row class="center-board-row"
          ...
          <el-form :size="formConf.size"
            ...
            <div v-show="!drawingList.length"
                 class="empty-info">
              <img src="@/assets/images/emptyElement.png"
                   alt=""
                   class="empty-img">
            </div>
          </el-form>
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (this.$store.getters.hasTable && config.layout === "colFormItem" && !noVModelList.includes(config.hszKey) && !e.__vModel__) {
              reject({ msg: `请选择${config.label}的控件字段`, target: 1 })
              break
            }
            if (config.hszKey === 'billRule') {
              if (!config.rule) {
                reject({ msg: '单据组件“选择模板”属性为必填项', target: 1 })
                break
              }
            }
            if (config.hszKey === 'relationForm') {
              if (!e.modelId) {
    ...
    shouldClone (to, from, target, event, conf) {
      ...
      if (conf.__config__.rowType === 'table') {
        if (noTableAllowList.includes(targetConf.__config__.hszKey)) {
          // if (this.showTip) {
    ...
    createIdAndKey (item) {
      ...
      if (config.layout === 'colFormItem') {
        if (!this.$store.getters.hasTable) {
          // 分割线和按钮不加vModel
          if (noVModelList.indexOf(config.hszKey) < 0) {
            item.__vModel__ = `${config.hszKey}Field${this.idGlobal}`
          }
        } else {
          if (noVModelList.indexOf(config.hszKey) < 0) {
            item.__vModel__ = ""
