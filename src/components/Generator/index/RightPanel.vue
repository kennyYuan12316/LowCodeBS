  ...
  <div class="right-board">
    ...
    <div class="field-box">
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab==='field' && showField"
          ...
          <template v-if="activeData.__config__">
            <template v-if="$store.getters.hasTable">
              <template v-if="activeData.__config__.hszKey==='table'">
                <el-form-item v-if="activeData.__vModel__!==undefined && !noVModelList.includes(activeData.__config__.hszKey)"
                              label="控件字段">
              ...
              <template v-else>
                <template v-if="!activeData.__config__.isSubTable">
                  <el-form-item v-if="activeData.__vModel__!==undefined && !noVModelList.includes(activeData.__config__.hszKey)"
                                label="数据库表">
                    <el-select v-model="activeData.__config__.tableName"
                               placeholder="请选择数据库表"
                               @change="tableChange"
                               filterable>
                      <el-option v-for="item in allTable"
                                 :key="item.table"
                                 :value="item.table"
                  ...
                  <el-form-item v-if="activeData.__vModel__!==undefined && !noVModelList.includes(activeData.__config__.hszKey)"
                                label="控件字段">
                    <el-select v-model="activeData.__vModel__"
                ...
                <template v-if="activeData.__config__.isSubTable && subTable.length">
                  <el-form-item v-if="activeData.__vModel__!==undefined && !noVModelList.includes(activeData.__config__.hszKey)"
                    ...
                    <el-select v-model="activeData.__vModel__"
                               ...
                               @change="fieldChange1"
                               filterable>
                      <el-option v-for="item in getSubTableFiled(activeData.__config__.relationTable)"
                                 :key="item.field"
                                 :value="item.field"
                                 :label="item.fieldName?item.field+'('+item.fieldName+')':item.field">
                      </el-option>
            ...
            <HSZComInput v-if="activeData.__config__.hszKey==='comInput'"
                         :active-data="activeData" />
            <HSZ-textarea v-if="activeData.__config__.hszKey==='textarea'"
                          :active-data="activeData" />
            <HSZText v-if="activeData.__config__.hszKey==='HSZText'"
                     :active-data="activeData" />
            <HSZNumInput v-if="activeData.__config__.hszKey==='numInput'"
                         :active-data="activeData" />
            <HSZAmount v-if="activeData.__config__.hszKey==='HSZAmount'"
                       :active-data="activeData" />
            <HSZPsdInput v-if="activeData.__config__.hszKey==='PsdInput'"
                         :active-data="activeData" />
            <HSZRadio v-if="activeData.__config__.hszKey==='radio'"
                      :active-data="activeData" />
            <HSZCheckbox v-if="activeData.__config__.hszKey==='checkbox'"
                         :active-data="activeData" />
            <HSZSelect v-if="activeData.__config__.hszKey==='select'"
                       :active-data="activeData"
                       :key="activeData.__config__.renderKey"
                       :dictionaryOptions="dictionaryOptions"
                       :dataInterfaceOptions="dataInterfaceOptions" />
            <HSZCascader v-if="activeData.__config__.hszKey==='cascader'"
                         :active-data="activeData" />
            <HSZTime v-if="activeData.__config__.hszKey==='time'"
                     :active-data="activeData" />
            <HSZTimeRange v-if="activeData.__config__.hszKey==='timeRange'"
                          :active-data="activeData" />
            <HSZDate v-if="activeData.__config__.hszKey==='date'"
                     :active-data="activeData" />
            <HSZDateRange v-if="activeData.__config__.hszKey==='dateRange'"
                          :active-data="activeData" />
            <HSZColorPicker v-if="activeData.__config__.hszKey==='colorPicker'"
                            :active-data="activeData" />
            <HSZRate v-if="activeData.__config__.hszKey==='rate'"
                     :active-data="activeData" />
            <HSZSwitch v-if="activeData.__config__.hszKey==='switch'"
                       :active-data="activeData" />
            <HSZSlider v-if="activeData.__config__.hszKey==='slider'"
                       :active-data="activeData" />
            <HSZDivider v-if="activeData.__config__.hszKey==='divider'"
                        :active-data="activeData" />
            <HSZUploadFz v-if="activeData.__config__.hszKey==='uploadFz'"
                         :active-data="activeData" />
            <HSZUploadImg v-if="activeData.__config__.hszKey==='uploadImg'"
                          :active-data="activeData" />
            <HSZComRight v-if="isCommon"
                         :active-data="activeData" />
            <HSZAddress v-if="activeData.__config__.hszKey==='address'"
                        :active-data="activeData" />
            <TreeSelect v-if="activeData.__config__.hszKey==='treeSelect'"
                        :active-data="activeData" />
            <GroupTitle v-if="activeData.__config__.hszKey==='groupTitle'"
                        :active-data="activeData" />
            <RelationForm v-if="activeData.__config__.hszKey==='relationForm'"
                          :active-data="activeData"
                          v-on="$listeners"
                          :key="activeData.__config__.renderKey" />
            <RelationFormAttr v-if="activeData.__config__.hszKey==='relationFormAttr'"
                              :active-data="activeData"
                              ref="relationFormAttr" />
            <HSZCalculate v-if="activeData.__config__.hszKey==='calculate'"
                          :active-data="activeData"
                          ref="calculate" />
            <PopupSelect v-if="activeData.__config__.hszKey==='popupSelect'"
                         :active-data="activeData" />
            <PopupAttr v-if="activeData.__config__.hszKey==='popupAttr'"
                       :active-data="activeData"
                       ref="popupAttr" />
            <Tab v-if="activeData.__config__.hszKey==='tab'"
                 :active-data="activeData" />
            <Collapse v-if="activeData.__config__.hszKey==='collapse'"
                      :active-data="activeData" />
            <template v-if="activeData.__config__.hszKey==='barcode'">
              ...
              <el-form-item label="宽度">
                <el-input-number v-model="activeData.width"
                                 placeholder="宽度"
                                 :min="0"
                                 :precision="0"
              ...
              <el-form-item label="默认值">
                <el-select v-model="activeData.dataType"
                           placeholder="请选择">
                  <el-option label="固定值"
                             value="static"></el-option>
                  <el-option label="组件联动"
                             value="relation"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="固定值"
                            v-if="activeData.dataType==='static'">
                <el-input v-model="activeData.staticText"
                          placeholder="请输入固定值"
                          @change="onBarcodeTextChange" />
              </el-form-item>
              <el-form-item label="选择组件"
                            v-if="activeData.dataType==='relation'">
                <el-select v-model="activeData.relationField"
                           placeholder="请选择"
                           clearable
                           filterable>
                  <el-option :label="item.__config__.label"
            ...
            <template v-if="activeData.__config__.hszKey==='qrcode'">
              <el-form-item label="控件标题">
                <el-input v-model="activeData.__config__.label"
                          placeholder="请输入控件标题" />
              </el-form-item>
              ...
              <el-form-item label="尺寸">
                <el-input-number v-model="activeData.size"
                                 placeholder="尺寸"
                                 :min="0"
                                 :precision="0"
                                 controls-position="right" />
            ...
            <template v-if="activeData.__config__.hszKey==='button'">
              ...
              <el-form-item label="样式">
                <el-select v-model="activeData.type"
                           placeholder="请选择">
                  <el-option label="默认按钮"
                             value=""></el-option>
                  <el-option label="主要按钮"
                             value="primary"></el-option>
                  <el-option label="成功按钮"
                             value="success"></el-option>
                  <el-option label="信息按钮"
                             value="info"></el-option>
                  <el-option label="警告按钮"
                             value="warning"></el-option>
                  <el-option label="危险按钮"
            ...
            <template v-if="isSystem">
              <el-form-item label="选择模板"
                            v-if="activeData.__config__.hszKey==='billRule'">
                <BillRule v-model="activeData.__config__.rule"
                          placeholder="选择模板" />
              </el-form-item>
              <el-form-item label="显示内容"
                            v-if="activeData.__config__.hszKey==='currOrganize'">
                <el-select v-model="activeData.showLevel"
                           placeholder="请选择显示内容">
                  <el-option label="显示组织"
                             value="all"></el-option>
                  <el-option label="显示部门"
            ...
            <template v-if="activeData.on && (modelType==1||modelType==6)">
              ...
              <div v-for="(value,key) in activeData.on"
                   :key="key">
                <el-form-item :label="key">
                  <el-button style="width: 100%;"
                             @click="editFunc(value,key)">
                    {{getTipText(key)}}
                  </el-button>
                </el-form-item>
              </div>
            ...
            <template v-if="activeData.__config__.hszKey==='table'">
              ...
              <el-form-item label="关联子表"
                            v-if="$store.getters.hasTable">
                <el-select v-model="activeData.__config__.tableName"
                           placeholder="请选择关联子表"
                           clearable
                           @change="onTableNameChange">
                  <el-option v-for="item in subTable"
                             :key="item.table"
                             :label="item.tableName?item.table+'('+item.tableName+')':item.table"
                             :value="item.table">
                  </el-option>
            ...
            <template v-if="activeData.__config__.hszKey==='card'">
              <el-form-item label="卡片标题"
                            label-width="76px">
                <el-input v-model="activeData.header"
                          placeholder="请输入卡片标题" />
              </el-form-item>
            ...
            <!-- row 布局 -->
            <template v-if="activeData.__config__.hszKey==='row'">
        ...
        <!-- 组件样式 -->
        <el-form v-show="currentTab === 'style'"
                 size="small"
                 label-width="90px"
                 labelPosition="left">
          <template v-if="activeData.__config__">
            <template v-if="!activeData.__config__.isSubTable">
              ...
              <el-form-item label="标题宽度"
                            v-if="!['divider','HSZText','button','table','groupTitle ','card','row','tab','collapse'].includes(activeData.__config__.hszKey)">
                <el-input-number v-model="activeData.__config__.labelWidth"
                                 placeholder="标题宽度"
                                 :min="0"
                                 :precision="0"
                                 controls-position="right" />
            ...
            <el-form-item label="控件宽度"
              ...
              <el-input-number v-model="activeData.__config__.columnWidth"
                               placeholder="控件宽度"
                               :min="0"
                               :precision="0"
                               controls-position="right" />
            ...
            <el-form-item label="多端显示">
              <el-checkbox-group v-model="activeData.__config__.visibility"
                                 size="mini"
        ...
        <el-form v-show="currentTab === 'form'"
          ...
          <el-form-item label="标签对齐">
            <el-radio-group v-model="formConf.labelPosition">
              ...
              <el-radio-button label="top"
                               :disabled="formConf.formStyle==='word-form'">顶部对齐
              </el-radio-button>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="标题宽度">
            <el-input-number v-model="formConf.labelWidth"
                             :min="0"
                             placeholder="标题宽度"
                             controls-position="right" />
          </el-form-item>
          ...
          <el-form-item label="弹窗类型"
            ...
            <el-select v-model="formConf.popupType"
                       placeholder="请选择">
              <el-option label="居中弹窗"
                         value="general"></el-option>
              <el-option label="右侧弹窗"
                         value="drawer"></el-option>
              <el-option label="全屏弹窗"
                         value="fullScreen"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="表单样式"
                        v-if="modelType != 3">
            <el-select v-model="formConf.formStyle"
                       placeholder="请选择">
              <el-option label="默认风格"
                         value=""></el-option>
              <el-option label="公文风格"
                         value="word-form"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="表单宽度"
                        v-if="formConf.popupType==='general'">
            <el-select v-model="formConf.generalWidth"
                       placeholder="请选择">
              <el-option v-for="item in generalWidthOptions"
                         :label="item"
                         :value="item"
                         :key="item" />
            </el-select>
          ...
          <el-form-item label="表单宽度"
            ...
            <el-select v-model="formConf.fullScreenWidth"
                       placeholder="请选择">
              <el-option v-for="item in fullScreenWidthOptions"
                         :label="item"
                         :value="item"
                         :key="item" />
            </el-select>
          ...
          <el-form-item label="表单宽度"
                        v-if="formConf.popupType==='drawer'">
            <el-select v-model="formConf.drawerWidth"
                       placeholder="请选择">
              <el-option v-for="item in generalWidthOptions"
                         :label="item"
                         :value="item"
          ...
          <template v-if="modelType!=3 && modelType!=6 && webType!=3">
            ...
            <div class="per-cell">
              <el-checkbox v-model="formConf.hasConfirmBtn"
                           disabled>确定</el-checkbox>
              <el-input v-model="formConf.confirmButtonText" />
            </div>
            <div class="per-cell">
              <el-checkbox v-model="formConf.hasCancelBtn"
                           disabled>取消</el-checkbox>
              <el-input v-model="formConf.cancelButtonText" />
            </div>
            <template v-if="webType==2">
              <div class="per-cell"
                   :class="{'last':!formConf.hasPrintBtn}">
                <el-checkbox v-model="formConf.hasPrintBtn">打印</el-checkbox>
                <el-input v-model="formConf.printButtonText" />
              </div>
              <el-form-item label=""
                            v-if="formConf.hasPrintBtn">
                <HSZ-TreeSelect :options="printTplList"
                                v-model="formConf.printId"
                                placeholder="请选择打印模板"
                                lastLevel
                                clearable></HSZ-TreeSelect>
              </el-form-item>
            </template>
          </template>
    ...
    <form-script :visible.sync="formScriptVisible"
                 :tpl="activeScript"
                 :fields="drawingList"
...
import HSZComInput from './RightComponents/ComInput'
import HSZTextarea from './RightComponents/Textarea'
import HSZText from './RightComponents/HSZText'
import HSZNumInput from './RightComponents/NumInput'
import HSZAmount from './RightComponents/HSZAmount'
import HSZPsdInput from './RightComponents/PsdInput'
import HSZRadio from './RightComponents/Radio'
import HSZCheckbox from './RightComponents/Checkbox'
import HSZSelect from './RightComponents/Select'
import HSZCascader from './RightComponents/Cascader/index'
import HSZTime from './RightComponents/Time'
import HSZTimeRange from './RightComponents/TimeRange'
import HSZDate from './RightComponents/Date'
import HSZDateRange from './RightComponents/DateRange'
import HSZColorPicker from './RightComponents/ColorPicker'
import HSZRate from './RightComponents/Rate'
import HSZSwitch from './RightComponents/Switch'
import HSZSlider from './RightComponents/Slider'
import HSZDivider from './RightComponents/Divider'
import HSZUploadFz from './RightComponents/UploadFz'
import HSZUploadImg from './RightComponents/UploadImg'
import HSZComRight from './RightComponents/ComRight'
import HSZAddress from './RightComponents/Address'
...
import HSZCalculate from './RightComponents/Calculate'
...
export default {
  ...
  computed: {
    ...
    isCommon () {
      return commonRightList.indexOf(this.activeData.__config__.hszKey) > -1
    },
    isSystem () {
      return systemList.indexOf(this.activeData.__config__.hszKey) > -1
    },
    drawingOptions () {
      let list = []
      const loop = (data, parent) => {
        if (!data) return
        if (data.__config__ && data.__config__.hszKey !== 'table' && data.__config__.children && Array.isArray(data.__config__.children)) {
          loop(data.__config__.children, data)
        }
        if (Array.isArray(data)) data.forEach(d => loop(d, parent))
        if (data.__vModel__ && data.__config__.hszKey !== 'table') {
          list.push(data)
        }
      }
  ...
  created () {
    ...
    if (!this.activeData.__config__.visibility) this.$set(this.activeData.__config__, "visibility", ["pc", "app"])
    if (!this.activeData.__config__.tableName && this.activeData.__config__.hszKey !== 'table') {
  ...
  methods: {
    ...
    fieldChange1 (val) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.hszKey === 'table' && config.tableName === this.activeData.__config__.relationTable) {
          ...
          if (config && config.hszKey != 'table' && config.children && Array.isArray(config.children)) {
    ...
    setDefaultOptions () {
      ...
      if (this.activeData.__vModel__ === undefined || this.activeData.__config__.hszKey === 'table') return
      if (!this.activeData.__config__.tableName || this.activeData.__config__.tableName === this.mainTable) {
        ...
        if (!list.length) {
          ...
          let options = item.fields.map(o => ({
            ...o,
            realField: 'hsz_' + this.activeData.__config__.tableName + '_hsz_' + o.field,
          }))
    ...
    onTableNameChange (tableName) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          const e = list[i]
          const config = e.__config__
          if (config.hszKey === 'table' && config.tableName === tableName) {
            boo = true
            break
          }
          if (config && config.hszKey != 'table' && config.children && Array.isArray(config.children)) {
