...
const layouts = {
  colFormItem (h, element, index, parent) {
    ...
    if (config.hszKey === 'barcode') {
      return (
        <el-col span={config.span} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <hsz-barcode format={element.format} lineColor={element.lineColor} background={element.background} width={element.width} height={element.height} staticText={element.staticText}></hsz-barcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.hszKey === 'qrcode') {
      return (
        <el-col span={config.span} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <hsz-qrcode size={element.size} colorLight={element.colorLight} colorDark={element.colorDark} staticText={element.staticText}></hsz-qrcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.hszKey === 'button') {
      return (
        <el-col span={config.span} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width="0">
            <hsz-button align={element.align} buttonText={element.buttonText} type={element.type} disabled={element.disabled}></hsz-button>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
  ...
  rowFormItem (h, element, index, parent) {
    ...
    const className = this.activeId === element.__config__.formId
      ...
      : 'drawing-row-item'
    if (element.__config__.hszKey === 'tab') {
      return (
    ...
    if (element.__config__.hszKey === 'collapse') {
      return (
        <el-col span={element.__config__.span}>
          <el-row gutter={element.__config__.gutter} class={className}
            nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
    ...
    if (element.__config__.hszKey === 'row') {
    ...
    if (element.__config__.hszKey === 'card') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
    ...
    if (element.__config__.hszKey === 'table') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
