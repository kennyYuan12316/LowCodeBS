<template>
  <el-dialog title="表单脚本"
             :close-on-click-modal="false"
             class="HSZ-dialog HSZ-dialog_center form-script-dialog"
             lock-scroll
             append-to-body
             v-bind="$attrs"
             width="1100px"
    ...
    <span slot="footer"
      ...
      <el-button type="primary"
                 @click="onClose()">{{$t('common.confirmButton')}}</el-button>
    </span>
  </el-dialog>
</template>

<script>
import HSZCodeEditor from '@/components/HSZEditor/monaco'
export default {
  components: { HSZCodeEditor },
  props: ['tpl', 'fields'],
  inject: ["getFormInfo"],
  data () {
    return {
  ...
  methods: {
    ...
    getTree () {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__vModel__) {
          if (data.__config__.hszKey === 'table') {
            let item = {
