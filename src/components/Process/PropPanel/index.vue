...
<template>
  <el-drawer size="550px"
             class="drawer HSZ-common-drawer"
    ...
    <header slot="title"
            class="header"
            v-else>{{properties.title}}</header>
        ...
        <el-row class="condition-list"
          ...
          <el-col :span="6"
            ...
            <el-select v-model="item.field"
              ...
              <el-option v-for="item in usedFormItems"
                         :key="item.__vModel__"
          ...
          <el-col :span="5">
            <el-select v-model="item.symbol"
              ...
              <el-option v-for="item in symbolOptions"
                         :key="item.value"
                         :label="item.label"
                         :value="item.value">
              </el-option>
          ...
          <el-col :span="6">
            <el-input v-model="item.filedValue"
    ...
    <section class="approver-pane"
      ...
      <el-tabs style="height:100%;">
            ...
            <el-form label-position="top"
              ...
              <el-form-item label="发起设置">
                <el-radio-group v-model="subFlowForm.initiateType">
                  <el-radio v-for="item in initiateTypeOptions"
                            :label="item.value"
                            :key="item.value"
                            :disabled="item.disabled"
                            class="radio-item">{{item.label}}
                  </el-radio>
                </el-radio-group>
                <div v-if="subFlowForm.initiateType === 1"
                     class="option-box-tip">
                  发起者的主管将作为子流程发起人</div>
                <div v-if="subFlowForm.initiateType === 2"
                     class="option-box-tip">
                  发起者的部门主管将作为子流程发起人</div>
                <div v-if="subFlowForm.initiateType === 3"
                     class="option-box-tip">
                  发起者自己将作为子流程发起人</div>
                <div v-if="subFlowForm.initiateType === 4"
                     class="option-box-tip">
                  选择表单字段的值作为子流程发起人</div>
                <div v-if="subFlowForm.initiateType === 5"
                     class="option-box-tip">
                  设置审批流程中某个环节的审批人作为子流程发起人</div>
                <div v-if="subFlowForm.initiateType === 6"
                     class="option-box-tip">
                  所指定的成员将作为子流程发起人，多人时，发起多个子流程</div>
                <div v-if="subFlowForm.initiateType === 9"
                     class="option-box-tip">
                  通过第三方调用从目标服务中获取子流程发起人</div>
                <el-form-item label="发起者的"
                ...
                <el-form-item label="表单字段"
                  ...
                  <el-select v-model="subFlowForm.formField"
                             placeholder="请选择字段">
                    <el-option v-for="item in usedFormItems"
                               :key="item.__vModel__"
                               :label="item.__config__.label"
                ...
                <el-form-item label="审批节点"
                  ...
                  <el-select v-model="subFlowForm.nodeId"
                             placeholder="请选择节点">
                ...
                <el-form-item style="margin-bottom:0;"
                  ...
                  <div slot="label">请求路径
                    <el-tooltip content="自带参数：taskId、taskNodeId"
                                placement="top">
                      <a class="el-icon-warning-outline"></a>
                    </el-tooltip>
                  </div>
                  <el-input v-model="subFlowForm.getUserUrl"
                            placeholder="请输入接口地址">
                ...
                <el-form-item style="margin-bottom:0;"
                  ...
                  <org-select ref="subFlow-role-org"
                              type="role"
                              v-model="subFlowForm.initiateRole"
                              title="添加角色"
                              class="mb-10"
                              buttonType="button" />
                  <org-select ref="subFlow-position-org"
                              buttonType="button"
                              v-model="subFlowForm.initiatePos"
                              title="添加岗位"
                              type="position"
                              class="mb-10" />
                  <org-select ref="subFlow-user-org"
                              buttonType="button"
                              v-model="subFlowForm.initiator"
                              title="添加用户" />
                </el-form-item>
              </el-form-item>
              ...
              <el-form-item label="子流程表单">
                <HSZ-TreeSelect :options="flowOptions"
                                v-model="subFlowForm.flowId"
                                placeholder="请选择子流程表单"
                                lastLevel
                                clearable
                                @change="subFlowForm.assignList=[]" />
              </el-form-item>
              <el-form-item label="数据传递">
                <div @click="openRuleBox">
                  <el-input :value="subFlowForm.assignList.length?'已设置':''"
                            placeholder="请设置数据传递规则"
        ...
        <el-tab-pane label="节点通知">
          <el-scrollbar class="config-scrollbar">
            <el-form :model="subFlowForm"
                     label-position="top"
                     class="pd-10">
              <el-alert title="该通知设置配置外部第三方消息提醒，站内信系统默认发送"
                        type="warning"
                        :closable="false"
                        show-icon />
              <el-form-item class="mt-10">
                <div slot="label">子流程发起
                  <el-tooltip content="该子流程被发起的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="subFlowForm.launchMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in noticeOptions"
                             :key="item.value"
                             :label="item.label"
                             :value="item.value" />
                </el-select>
              ...
              <template v-if="subFlowForm.launchMsgConfig.on===1">
                ...
                <el-table :data="subFlowForm.launchMsgConfig.templateJson">
                  ...
                  <el-table-column prop="value"
                                   label="表单字段">
                    <template slot-scope="scope">
                      <el-select v-model="scope.row.relationField"
                                 placeholder="请选择表单字段"
                                 clearable
                                 filterable>
      ...
      <el-tabs style="height:100%;">
        <el-tab-pane label="基础设置"
          ...
          <el-scrollbar class="config-scrollbar">
            <el-row class="pd-10">
              <el-col :span="4"
                      style="font-size: 14px;line-height:32px">谁可以发起</el-col>
              <el-col :span="20"
                      style="font-size: 14px;line-height:32px;margin-bottom:10px">
                默认所有人,需要设置请选择
              </el-col>
              <div class="option-box">
                <org-select ref="start-role-org"
                            type="role"
                            v-model="initiateRole"
                            title="添加角色"
                            class="mb-10"
                            buttonType="button" />
                <org-select ref="start-position-org"
                            type="position"
                            v-model="initiatePos"
                            title="添加岗位"
                            class="mb-10"
                            buttonType="button" />
                <org-select ref="start-user-org"
                            type="user"
                            v-model="initiator"
                            title="添加用户"
                            buttonType="button" />
          ...
          <el-scrollbar class="config-scrollbar">
            <el-form label-position="top"
              ...
              <el-form-item label="操作设置">
                <div class="per-cell">
                  <el-checkbox v-model="startForm.hasSubmitBtn"
                               disabled>提交</el-checkbox>
                  <el-input v-model="startForm.submitBtnText" />
                </div>
                <div class="per-cell">
                  <el-checkbox v-model="startForm.hasSaveBtn"
                               disabled>暂存</el-checkbox>
            ...
            <el-form class="pd-10"
              ...
              <el-form-item label="审核汇总">
                ...
                <div class="option-box-tip"
                     v-if="!startForm.isSummary">*打开审核汇总后，流程流转记录会按部门及岗位进行汇总展示
                </div>
              </el-form-item>
              <el-form-item label="汇总设置"
                            v-if="startForm.isSummary">
                <el-select v-model="startForm.summaryType"
                           placeholder="请选择">
                  <el-option label="汇总全部流转记录"
                             :value="0"></el-option>
                  <el-option label="汇总通过及拒绝流转记录"
                             :value="1"></el-option>
                </el-select>
              ...
              <el-table-column prop="write"
                ...
                <template slot-scope="scope">
                  ...
                  <el-checkbox v-model="scope.row.required"
                               :disabled="scope.row.requiredDisabled">
                    必填</el-checkbox>
                </template>
              </el-table-column>
            </el-table>
                    ...
                    <template slot-scope="scope">
                      <template v-if="scope.row.required">
                        <el-select v-model="scope.row.relationField"
                                   ...
                                   clearable
                                   filterable>
                          <el-option v-for="item in funcRequiredOptions"
                                     :key="item.__vModel__"
                                     :label="item.__config__.label?item.__vModel__+'('+item.__config__.label+')':item.__vModel__"
                                     :value="item.__vModel__">
                          </el-option>
        ...
        <el-tab-pane label="流程通知">
          <el-scrollbar class="config-scrollbar">
            <el-form :model="startForm"
              ...
              <el-form-item class="mt-10">
                <div slot="label">流程待办
                  <el-tooltip content="流程处于等待的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="startForm.waitMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in noticeOptions"
                             :key="item.value"
                             :label="item.label"
              ...
              <el-form-item>
                <div slot="label">流程结束
                  <el-tooltip content="流程结束的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="startForm.endMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in noticeOptions"
                             :key="item.value"
                             :label="item.label"
              ...
              <el-form-item>
                <div slot="label">节点同意
                  <el-tooltip content="所有节点审核人同意的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="startForm.approveMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in noticeOptions"
                             :key="item.value"
                             :label="item.label"
              ...
              <el-form-item>
                <div slot="label">节点拒绝
                  <el-tooltip content="所有节点审核人拒绝的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="startForm.rejectMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in noticeOptions"
                             :key="item.value"
                             :label="item.label"
              ...
              <el-form-item>
                <div slot="label">节点抄送
                  <el-tooltip content="所有节点抄送的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="startForm.copyMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in noticeOptions"
                             :key="item.value"
                             :label="item.label"
            ...
            <el-form label-position="top"
              ...
              <el-form-item label="审批设置">
                <el-radio-group v-model="approverForm.assigneeType"
                                @change="resetOrgColl">
                  <el-radio v-for="item in assigneeTypeOptions"
                            :label="item.value"
                            :key="item.value"
                            :disabled="item.disabled"
                            class="radio-item">{{item.label}}
                ...
                <div v-if="approverForm.assigneeType === 1"
                     class="option-box-tip">
                  发起者主管将作为审批人处理审批单</div>
                <div v-if="approverForm.assigneeType === 2"
                     class="option-box-tip">
                  发起者的部门主管将作为审批人处理审批单</div>
                <div v-if="approverForm.assigneeType === 3"
                     class="option-box-tip">
                  发起者自己将作为审批人处理审批单</div>
                <div v-if="approverForm.assigneeType === 4"
                     class="option-box-tip">
                  选择流程表单字段的值作为审批人</div>
                <div v-if="approverForm.assigneeType === 5"
                     class="option-box-tip">
                  设置审批人为审批流程中某个环节的审批人</div>
                <div v-if="approverForm.assigneeType === 6"
                     class="option-box-tip">
                  指定审批人处理审批单</div>
                <div v-if="approverForm.assigneeType === 7"
                     class="option-box-tip">
                  指定可供选择的候选人处理审批单</div>
                <div v-if="approverForm.assigneeType === 9"
                     class="option-box-tip">
                ...
                <el-form-item label="表单字段"
                  ...
                  <el-select v-model="approverForm.formField"
                             placeholder="请选择字段">
                    <el-option v-for="item in usedFormItems"
                               :key="item.__vModel__"
                               :label="item.__config__.label"
                ...
                <el-form-item label="审批节点"
                  ...
                  <el-select v-model="approverForm.nodeId"
                             placeholder="请选择节点">
                    <el-option v-for="item in nodeOptions"
                               :key="item.nodeId"
                               :label="item.properties.title"
                ...
                <el-form-item style="margin-bottom:0;"
                  ...
                  <el-input v-model="approverForm.getUserUrl"
                            placeholder="请输入接口地址">
                    <template slot="prepend">GET</template>
                  </el-input>
                </el-form-item>
                <el-form-item style="margin-bottom:0;"
                  ...
                  <org-select ref="approver-role-org"
                              type="role"
                              title="添加角色"
                              buttonType="button"
                              v-model="approverForm.approverRole"
                              class="mb-10" />
                  <org-select ref="approver-position-org"
                              buttonType="button"
                              v-model="approverForm.approverPos"
                              title="添加岗位"
                              type="position"
                              class="mb-10" />
                  <org-select ref="approver-user-org"
                              title="添加用户"
                              buttonType="button"
              ...
              <el-form-item label="审批方式">
                <el-radio v-model="approverForm.counterSign"
                          :label="0">
              ...
              <el-form-item>
                <div slot="label">加签设置
                  <el-tooltip content="开启后审批单可以新增临时审批人"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
              ...
              <el-form-item label="进度设置">
                <el-select v-model="approverForm.progress"
                           placeholder="请选择">
              ...
              <el-form-item label="抄送设置">
                <org-select ref="approver-copy-role-org"
                            type="role"
                            v-model="approverForm.circulateRole"
                            title="添加角色"
                            class="mb-10"
                            buttonType="button" />
                <org-select ref="approver-copy-position-org"
                            ...
                            type="position"
                            class="mb-10" />
                <org-select ref="approver-copy-user-org"
                            buttonType="button"
          ...
          <el-scrollbar class="config-scrollbar">
            <el-form label-position="top"
              ...
              <el-form-item label="操作设置">
                ...
                <div class="per-cell"
                     v-if="approverForm.hasPrintBtn">
                  <p style="width:112px"></p>
                  <HSZ-TreeSelect :options="printTplList"
                                  v-model="approverForm.printId"
                                  placeholder="请选择打印模板"
                                  lastLevel
                                  clearable></HSZ-TreeSelect>
                </div>
              ...
              <el-form-item label="驳回设置">
                <el-select v-model="approverForm.rejectStep"
                           placeholder="请选择">
                  <el-option v-for="item in rejectStepOptions"
                             :key="item.nodeId"
                             :label="item.properties.title"
                             :value="item.nodeId">
                  </el-option>
              ...
              <el-form-item label="说明">
                <el-input v-model="approverForm.description"
                          type="textarea"
                          :rows="3"></el-input>
              </el-form-item>
            </el-form>
        ...
        <el-tab-pane label="节点通知">
          <el-scrollbar class="config-scrollbar">
            <el-form :model="approverForm"
              ...
              <el-form-item class="mt-10">
                <div slot="label">节点同意
                  <el-tooltip content="当前节点审核人同意的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="approverForm.approveMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in nodeNoticeOptions"
                             :key="item.value"
                             :label="item.label"
                             :value="item.value" />
                </el-select>
              ...
              <el-form-item>
                <div slot="label">节点拒绝
                  <el-tooltip content="当前节点审核人拒绝的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="approverForm.rejectMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in nodeNoticeOptions"
                             :key="item.value"
                             :label="item.label"
              ...
              <el-form-item>
                <div slot="label">节点抄送
                  <el-tooltip content="当前节点抄送的时候"
                              placement="top">
                    <a class="el-icon-warning-outline"></a>
                  </el-tooltip>
                </div>
                <el-select v-model="approverForm.copyMsgConfig.on"
                           placeholder="请选择">
                  <el-option v-for="item in nodeNoticeOptions"
                             :key="item.value"
                             :label="item.label"
    ...
    <el-dialog title="数据传递"
               :close-on-click-modal="false"
               :visible.sync="ruleVisible"
               class="HSZ-dialog HSZ-dialog_center rule-dialog"
               lock-scroll
               append-to-body
               width='700px'>
      <div class="option-box-tip">当父流程流转到子流程时，将对应的字段赋值给子流程</div>
      <el-row :gutter="10"
              v-for="(item,i) in assignList"
              :key="i"
              class="mb-10">
        <el-col :span="2"
                class="rule-cell">父流程</el-col>
        <el-col :span="7"
                class="rule-cell">
          <el-select v-model="item.parentField"
                     placeholder="请选择字段">
            <el-option v-for="item in usedFormItems"
                       :key="item.__vModel__"
                       :label="item.__config__.label"
        ...
        <el-col :span="4"
                class="rule-cell mid">赋值给</el-col>
        <el-col :span="2"
                class="rule-cell">子流程</el-col>
        <el-col :span="7"
                class="rule-cell">
          <el-select v-model="item.childField"
                     placeholder="请选择字段">
            <el-option v-for="item in childFieldOptions"
                       :key="item.vmodel"
                       :label="item.label"
                       :value="item.vmodel" />
          </el-select>
      ...
      <div class="table-actions"
           @click="addRule">
        <el-button type="text"
                   icon="el-icon-plus">新增规则</el-button>
      </div>
      <span slot="footer"
            class="dialog-footer">
        <el-button @click="ruleVisible = false">取消</el-button>
        <el-button type="primary"
                   @click="saveRule">确定</el-button>
      </span>
    </el-dialog>
  </el-drawer>
</template>
...
},
{
  ...
  __vModel__: 'hszFlowOperatorUserId',
},
{
  __config__: {
    label: '当前操作用户名',
  ...
  __vModel__: 'hszFlowOperatorUserName',
}]
export default {
  ...
  computed: {
    ...
    usedFormItems () {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__vModel__ && data.__config__.hszKey !== 'table') list.push(data)
      }
      loop(getDrawingList())
