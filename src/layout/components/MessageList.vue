<template>
  <div>
    <el-drawer title="站内消息"
               :visible.sync="drawer"
               direction="rtl"
               size="280px"
               class="HSZ-messageList HSZ-common-drawer"
               :before-close="handleClose">
      <div class="tool">
        <el-link :underline="false"
                 @click.native="readAll">全部已读</el-link>
        <el-link :underline="false"
                 @click.native="gotoCenter">消息中心</el-link>
      </div>
      <div class="HSZ-messageList-box"
           v-loading="loading && listQuery.currentPage==1"
           ref="messageListBody">
        <div v-if="list.length">
          <div v-for="(item,i) in list"
               :key="i"
               class="HSZ-messageList-item"
               @click="readInfo(item)"
               :title="item.title">
            <el-badge is-dot
                      :hidden="item.isRead=='1'"
                      type="warning">
              <i class="el-icon-chat-dot-square HSZ-messageList-item-icon"></i>
            </el-badge>
            <div class="HSZ-messageList-txt">
        ...
        <p class="noData-txt"
           v-else>{{$t('common.noData')}}</p>
      </div>
    </el-drawer>
    <el-dialog title="查看消息"
               :close-on-click-modal="false"
               :visible.sync="visible"
               class="HSZ-dialog HSZ-dialog_center HSZ-dialog-notice"
               lock-scroll
               width="80%">
      <div class="notice-wrapper"
        ...
        <div class="main"
             v-html="info.bodyText"></div>
        <div class="file-list"
             v-if="files.length">
          <HSZ-UploadFz v-model="files"
                        disabled
                        detailed />
        </div>
      </div>
