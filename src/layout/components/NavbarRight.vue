<template>
  <div class="right-menu">
    <template v-if="device!=='mobile'">
      ...
      <el-tooltip :content="$t('navbar.chat')"
                  placement="bottom">
        <i class="icon-sz icon-sz-header-IM right-menu-item hover-effect"
           @click="openUserList()"
      ...
      <el-tooltip :content="$t('navbar.notice')"
        ...
        <div class="right-menu-item hover-effect"
          ...
          <el-badge :value="messageCount"
                    :hidden="!messageCount"
                    :max="99">
            <i class="icon-sz icon-sz-header-message"></i>
          </el-badge>
        </div>
      </el-tooltip>
    ...
    <el-dropdown class="avatar-container right-menu-item hover-effect"
      ...
      <el-dropdown-menu slot="dropdown">
        <router-link to="/profile">
          <el-dropdown-item>
            <i class="icon-sz icon-sz-header-userInfo"></i>{{ $t('navbar.profile') }}
          </el-dropdown-item>
        </router-link>
        <a href="http://mail.qq.com/cgi-bin/qm_share?t=qm_mailme&email=JU1WXwsXFRQXZUNKXUhETEkLRkpI"
           target="_blank">
          <el-dropdown-item>
            <i class="icon-sz icon-sz-header-feedBack"></i>{{ $t('navbar.feedback') }}
          </el-dropdown-item>
        </a>
        <el-dropdown-item @click.native="visible = true">
          <i class="icon-sz icon-sz-header-about"></i>{{ $t('navbar.about') }}
        </el-dropdown-item>
        <el-dropdown-item @click.native="visible2 = true">
          <i class="icon-sz icon-sz-generator-card"></i>{{ $t('navbar.statement') }}
        </el-dropdown-item>
        <el-dropdown-item @click.native="lock">
          <i class="icon-sz icon-sz-header-lockScreen"></i>{{ $t('navbar.lockScreen') }}
        </el-dropdown-item>
        <el-dropdown-item divided
                          @click.native="handleLogout">
          <i class="icon-sz icon-sz-header-loginOut"></i>{{ $t('navbar.logOut') }}
        </el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
    <el-tooltip :content="$t('navbar.setting')"
                placement="bottom">
      <div class="right-menu-item hover-effect"
           @click="$refs.Settings.init()"
           v-if="showSettings">
        <i class="icon-sz icon-sz-header-pannel"></i>
      </div>
    </el-tooltip>
    <el-dialog title="关于平台"
               :close-on-click-modal="false"
               :visible.sync="visible"
               v-drag-dialog
               class="HSZ-dialog HSZ-dialog_center about-dialog"
               lock-scroll
               width="400px">
      <div class="about-dialog-main">
        <div>
          <img :src="define.comUrl+sysConfig.logoIcon"
               class="about-logo"
        ...
        <div>
          <p class="title"><a href="https://www.hszsoft.com"
    ...
    <el-dialog title="官方声明"
      ...
      <div class="about-dialog-main">
        <div>
          开发平台是对外开放性体验产品，平台内所有数据为我司虚构的测试数据，是随机生成非敏感数据，所以严禁任何人在开发平台上修改数据内容、扭曲数据目的、传播或售卖测试数据，特此警告。
        </div>
      </div>
    </el-dialog>
      ...
      if ('WebSocket' in window) {
        ...
        socket.onmessage = (event) => {
          ...
          if (data.method == 'receiveMessage') {
            //判断是否打开窗口
            if (this.$refs.UserList && this.$refs.UserList.$refs.HSZIm && this.$refs.UserList.$refs.HSZIm.visible) {
              if (this.$refs.UserList.$refs.HSZIm.info.id === data.formUserId) {
                let messIitem = {
                  userId: data.formUserId,
                  messageType: data.messageType,
                  message: data.formMessage,
                  dateTime: this.hsz.toDate(data.dateTime)
                }
                this.$refs.UserList.$refs.HSZIm.addItem(messIitem)
                //更新已读
          ...
          if (data.method == 'sendMessage') {
            if (this.$refs.UserList.$refs.HSZIm.info.id !== data.toUserId) return
            //添加到客户端
            let messIitem = {
              userId: data.UserId,
              messageType: data.messageType,
          ...
          if (data.method == 'messageList') {
            this.$refs.UserList.$refs.HSZIm.getList(data)
          }
        }
      }
    },
...
.about-dialog {
  &.HSZ-dialog.HSZ-dialog_center >>> .el-dialog .el-dialog__body {
    padding: 0 0 40px 0;
  }
  .about-dialog-main {
    display: flex;
